...
{
    public partial class frmMachinery_1 : RosySystem.Customize.frmView
    {
        ...
        DataTable dtDmTb = new DataTable();
        BindingSource bdsDmTb = new BindingSource();
        rsDataGridView dgvDmTb = new rsDataGridView();
        DataRow drDmTb;

        DataTable dtDmTbMoTa = new DataTable();
        BindingSource bdsDmTbMoTa = new BindingSource();
        ...
        {
			if (tabDetail.SelectedTab == tabVTPT)
				deletePTKT();
        ...
        {
            if (tabDetail.SelectedTab == tabMoTaChung)
                EditMoTaChung(enuEdit.Edit);
            else if (tabDetail.SelectedTab == tabVTPT)
				EditPTKT(enuEdit.Edit);
            //else if (tabDetail.SelectedTab == tabVTDP)
        ...
        {
            if (bdsDmTb.Position < 0)
                return;
            if (bdsDmTbMoTa.Position < 0 && enuNew_Edit == enuEdit.Edit)
                return;

            drDmTbMoTa = ((DataRowView)bdsDmTbMoTa.Current).Row;

            //Copy dong hien tai
            if (bdsDmTbMoTa.Position >= 0)
                Common.CopyDataRow(((DataRowView)bdsDmTbMoTa.Current).Row, ref drCurrent);
            else
                drCurrent = dtDmTbMoTa.NewRow();

            drCurrent["Ma_Tb"] = drDmTb["Ma_Tb"];
            ...
            if (frmEdit.isAccept)
            {
               if (enuNew_Edit == enuEdit.New)
                {
                    if (bdsDmTbMoTa.Position >= 0)
                        dtDmTbMoTa.ImportRow(drCurrent);
                    else
                        dtDmTbMoTa.Rows.Add(drCurrent);

                    bdsDmTbMoTa.Position = bdsDmTbMoTa.Find("Ident00", drCurrent["Ident00"]);
                }
                else
                   Common.CopyDataRow(drCurrent, ((DataRowView)bdsDmTbMoTa.Current).Row);

               dtDmTbMoTa.AcceptChanges();
            }
            else
                dtDmTbMoTa.RejectChanges();
        }
        ...
        private void FilterDetail()
        {
            ...
            drDmVtTb = ((DataRowView)bdsDmVtTb.Current).Row;
            bdsDmTbMoTa.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            bdsPTKT.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            bdsVTDP.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            bdsBTSC.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            bdsResource.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            bdsKHBTSC.Filter = "Ma_Tb = '" + (string)drDmTb["Ma_Tb"] + "'";
            loadImage();
        }
        ...
        void bdsDmNhVtTb_PositionChanged(object sender, EventArgs e)
        {
            ...
            if ((bool)drDmNhVtTb["Nh_Cuoi"] == false)
                bdsDmTb.Filter = "Ma_Nh_Tb_Parent = '" + (string)drDmNhVtTb["Ma_Nh_Tb"] + "'";
            else
                bdsDmTb.Filter = "Ma_Nh_Tb = '" + (string)drDmNhVtTb["Ma_Nh_Tb"] + "'";

            tabDmTb.SelectedTab = tabDmVtTb;
            this.FilterDetail();
        }
        ...
        private void Build()
        {
            ...
            dgvDmTb.strZone = "DMTB";
            dgvDmTb.BuildGridView(this.isLookup);
            this.tabDmVtTb.Controls.Add(dgvDmTb);

            dgvMoTaChung.Dock = DockStyle.Fill;
            dgvMoTaChung.strZone = "DMTBMOTA";
            dgvMoTaChung.BuildGridView();
            this.tabMoTaChung.Controls.Add(dgvMoTaChung);

            //dgvDmVtTb.Dock = DockStyle.Fill;
            //dgvDmVtTb.strZone = "DMVTTB";
            //dgvDmVtTb.BuildGridView(this.isLookup);
            //this.pagePTKT.Controls.Add(dgvDmVtTb);
        ...
        private void FillData()
        {
            ...
            //Danh mục thiết bị
            dtDmTb = dsMachinery.Tables[1];
            bdsDmTb.DataSource = dtDmTb;
            dgvDmTb.DataSource = bdsDmTb;

            //Danh mục thiết bị mô tả
            dtDmTbMoTa = dsMachinery.Tables[2];
            bdsDmTbMoTa.DataSource = dtDmTbMoTa;
            dgvMoTaChung.DataSource = bdsDmTbMoTa;

            ////Danh mục vật tư thiết bị
            //dtDmVtTb = dsMachinery.Tables[3];
        ...
        {
            if (bdsDmTb.Position < 0 && enuNew_Edit == enuEdit.Edit)
                return;

            //Copy dòng hiện tại
            if (bdsDmTb.Position >= 0)
                Common.CopyDataRow(((DataRowView)bdsDmTb.Current).Row, ref drDmTb);
            else
                drDmTb = dtDmTb.NewRow();

            if (enuNew_Edit == enuEdit.New)
                drDmTb["Ma_Nh_Tb"] = drDmNhVtTb["Ma_Nh_Tb"];

            frmMachinery_Edit frmEdit = new frmMachinery_Edit();
            frmEdit.Load(enuNew_Edit, drDmTb);
            ...
            if (frmEdit.isAccept)
            {
                ...
                {
                    if (bdsDmTb.Position >= 0)
                        dtDmTb.ImportRow(drDmTb);
                    else
                        dtDmTb.Rows.Add(drDmTb);

                    bdsDmTb.Position = bdsDmTb.Find("Ma_Vt", drDmTb["Ma_Vt"]);
                }
                else
                    Common.CopyDataRow(drDmTb, ((DataRowView)bdsDmTb.Current).Row);
                ...
                dtDmTb.AcceptChanges();
            }
            else
                dtDmTb.RejectChanges();
        }
