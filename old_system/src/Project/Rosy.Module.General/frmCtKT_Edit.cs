	...
	public partial class frmCtKT_Edit : frmVoucher_Edit
	{
		...
		private void Init_Ct()
		{
			...
			if (this.enuNew_Edit == enuEdit.New || this.enuNew_Edit == enuEdit.Copy)
			{
				...
				drCurrent["Gia_PP"] = false;
				drCurrent["Ma_Ky_Hieu_Hdon"] = "";
			}
		...
		private void LoadDicName()
		{
			...
			((dgvTextBoxColumn)dgvEditCt1.Columns["Ma_So_Thue"]).bUseAutoDropDown = true;
		...
		private void DataGridView_Language()
		{
			if (dgvEditCt1.Columns.Contains("Ma_Vt"))
				dgvEditCt1.Columns["Ma_Vt"].HeaderText = "Mã Doanh thu/Chi phí";
			if (dgvEditCt1.Columns.Contains("Ma_Dt_Co"))
				dgvEditCt1.Columns["Ma_Dt_Co"].HeaderText = "Mã đối tượng có";
			if (dgvEditCt1.Columns.Contains("Ma_Dt"))
				dgvEditCt1.Columns["Ma_Dt"].HeaderText = "Mã đối tượng nợ";
			if (dgvEditCt1.Columns.Contains("Han_Tt"))
				dgvEditCt1.Columns["Han_Tt"].HeaderText = "Hạn thanh toán nợ";
		}
		//private string GetNotice()
  //      {
		//	drCurrent = ((DataRowView)bdsEditCt.Current).Row;
		//	string strNotice = "";
		//	if(drCurrent["Ma_Hd"].ToString() != string.Empty)
		//		strNotice += " Hợp đồng:" + Voucher.GetTenHd(drCurrent["Ma_Hd"].ToString());
		//	if (drCurrent["Ma_Dt"].ToString() != string.Empty)
		//		strNotice += " Đối tượng:" + DataTool.SQLGetNameByCode("R81DMDT", "Ma_Dt", "Ten_Dt", drCurrent["Ma_Dt"].ToString());
		//	if (drCurrent["Ma_Dt_Co"].ToString() != string.Empty)
		//		strNotice += " Đối tượng có:" + DataTool.SQLGetNameByCode("R81DMDT", "Ma_Dt", "Ten_Dt", drCurrent["Ma_Dt_Co"].ToString());
		...
		private bool FormCheckValid()
		{
			...
			foreach (DataRow dr in dtEditCt.Rows)
			{
				...
				//if (Common.InlistLike(dr["Tk_No"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_Co"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_No3"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_Co3"].ToString(), strTk_Not_Sl))
				//{ 
				//}
				...
				if (Common.InlistLike(dr["Tk_No"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_Co"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_No3"].ToString(), strTk_Not_Sl) || Common.InlistLike(dr["Tk_Co3"].ToString(), strTk_Not_Sl))
                {
                    ...
                    strMsg = "Vui lòng không hạch toán tài khoản loại 15 trên phiếu kế toán ";

                    Common.MsgCancel(strMsg);
                    return false;
                }
                DataRow drTk_No = DataTool.SQLGetDataRowByID("R81DMTK", "Tk", dr["Tk_No"].ToString());
                DataRow drTk_Co = DataTool.SQLGetDataRowByID("R81DMTK", "Tk", dr["Tk_Co"].ToString());
				//if (dr["Ma_Dt"].ToString() == "" && dr["Ma_Dt_Co"].ToString() == "")
    //            {
				//	Common.MsgCancel("Mã đối tượng không thể đồng thời rỗng");
				//	return false;
				//}
				//if ( dr["Ma_Dt_Co"].ToString() != "")
    //            {
				//	string strMsg = string.Empty;
					
				//	if (!(bool)drTk_Co["Tk_Dt"])
				//		strMsg = "Vui lòng xóa mã đối tượng có vì tài khoản có không cần mã đối tượng";
    ...
    //            }
				...
				string strTk_Cn = (string)RosySystem.Library.Parameters.GetParaValue("TK_HANTT_LIST");// ("TK_HANTT_LIST");
				#region Kiểm tra hạch toán hợp lệ của Tk_No
				if (drDmTkNo != null)
				{
                    //Kiểm tra tài khoản nợ chưa nhập hạn thanh toán
                    if (Common.InlistLike(dr["Tk_No"].ToString(), strTk_Cn) && dr["Tk_No"].ToString().StartsWith ("1") && Convert.ToDouble(dr["Han_Tt"]) == 0 
						&& Convert.ToDouble(dr["Tien_Nt9"]) > 0)
					{
                        string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Tk nợ yêu cầu nhập hạn thanh toán" : "Debit account require Customer code";
                        Common.MsgOk(strMsg);
                        return false;
                    }
					...
					{
						string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Tk có yêu cầu Mã khoản mục" : "Debit account require Category code";
						Common.MsgCancel(strMsg);
						return false;
					}
					if ((dr["Tk_No"].ToString().StartsWith("511") || dr["Tk_Co"].ToString().StartsWith("511") || dr["Tk_No"].ToString().StartsWith("632") || dr["Tk_Co"].ToString().StartsWith("632")) 
							&& dr["Ma_Vt"].ToString() == "")
					{
						string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Nghiệp vụ yêu cầu nhập mã doanh thu/chi phí" : "Debit account require Category code";
						Common.MsgCancel(strMsg);
						return false;
					}
				}
				...
				if (drDmTkCo != null)
				{
                    if (Common.InlistLike(dr["Tk_Co"].ToString(), strTk_Cn) && dr["Tk_Co"].ToString().StartsWith("3") && Convert.ToDouble(dr["Han_Tt_Co"]) == 0 
							&& Convert.ToDouble(dr["Tien_Nt9"]) > 0 )
                    {
                        string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Tk có yêu cầu nhập hạn thanh toán" : "Debit account require Customer code";
                        Common.MsgOk(strMsg);
                        return false;
                    }

					if ((bool)drDmTkCo["Tk_Dt"] && dr["Ma_Dt_Co"].ToString() == "")
					{
				...
				if(!Voucher.CheckThue(dr["So_Ct0"].ToString(), dr["So_Seri0"].ToString(), dr["Ma_So_Thue"].ToString(), dr["Ma_Thue"].ToString(), dr["Ngay_Ct0"].ToString(),
						Convert.ToDouble(dr["Tien_Nt9"]), Convert.ToDouble(dr["Tien_Nt3"])))
					return false;
				
				//kiểm tra ngày hóa đơn sau ngày 
				if (((Library.StrToDate(dr["Ngay_Ct0"].ToString()).Month != Library.StrToDate(dteNgay_Ct.Text).Month && 
					Library.StrToDate(dr["Ngay_Ct0"].ToString()).Year == Library.StrToDate(dteNgay_Ct.Text).Year) && dr["Ma_Thue"].ToString() != "VKK")
					|| (dr["Ma_Thue"].ToString() == "VKK" && 
					Library.StrToDate(dr["Ngay_Ct0"].ToString()).Year + Library.StrToDate(dr["Ngay_Ct0"].ToString()).Month > Library.StrToDate(dteNgay_Ct.Text).Year + Library.StrToDate(dteNgay_Ct.Text).Month))
				{
					Common.MsgOk("Ngày hóa đơn tại dòng số hóa đơn " + dr["So_Ct0"].ToString() + " đang lớn hơn hoặc bé hơn ngày chứng từ. Anh (chị) sửa ngày hóa đơn phải cùng tháng năm với ngày chứng từ!!");
					return false;
				}
				//kiễm tra số tờ khai
				//if (Common.InlistLike(dr["Tk_No"].ToString(), "151") && Common.InlistLike(dr["Tk_Co"].ToString(), "3312")  && dr["So_TKhai"].ToString() == "")
				//{
				//	Common.MsgCancel("Nghiệp vụ hàng nhập khẩu yêu cầu nhập số tờ khai");
				//	return false;
				//}
		...
		public override bool Save()
		{
			...
			// xử lý ngầm cột Dien_Giai VÀ MÃ DT
			//if (enuNew_Edit == enuEdit.New || ENU)//drEditPh["Dien_Giai"].ToString() == string.Empty)
			//{
				drEditPh["Dien_Giai"] = dtEditCt.Rows[0]["Dien_Giai"].ToString();
			
			if(dtEditCt.Rows[0]["Ma_Dt"].ToString() != string.Empty)	
				drEditPh["Ma_Dt"] = dtEditCt.Rows[0]["Ma_Dt"].ToString();
			else if (dtEditCt.Rows[0]["Ma_Dt_Co"].ToString() != string.Empty)
				drEditPh["Ma_Dt"] = dtEditCt.Rows[0]["Ma_Dt_Co"].ToString();
			//}
		...
		{//Ham thuc hien phim Enter: true: thuc hien thanh cong, false: khong thuc hien duoc
			...
			if (Common.Inlist(strCurrentColumn, "TK_NO,TK_CO"))
			{
				...
				if (dgvCell.FormattedValue.ToString() == string.Empty)
				{
					...
					{
						if (!dgvEditCt1.bIsCurrentFirstRow) //Chỉ remove khi kô fải là dòng đầu tiên
						{
			...
			if (Common.Inlist(strCurrentColumn, "TK_CO3")) //MA_THUE
			{
				if (dgvCell.FormattedValue.ToString() == string.Empty)
				{
			...
			if (Common.Inlist(strCurrentColumn, strColumnLast)) //"TK_CO3"
			{
				if (dgvEditCt1.bIsCurrentLastRow)
				{
					if (!Voucher.AddRow(this))
        ...
        //    DataRow drLookup = Lookup.ShowLookup("Ma_Dt_Hq", strValue, bRequire, "Ma_Nh_Dt = 'HQ'");
		...
		void dgvEditCt_CellEnter(object sender, DataGridViewCellEventArgs e)
		{
            ...
            else if (Common.Inlist(strColumnName, "MA_DT, MA_DT_CO, MA_HD, MA_KHO"))
            {
                ((dgvTextBoxColumn)dgvEditCt.Columns[strColumnName]).bUseAutoDropDown = true;

                //if ((string)drCurrent[strColumnName] != string.Empty) //&& Common.CheckPermissionTk(drCurrent[strColumnName].ToString(), enuPermission_Type.Allow_View))
                //{
                    //this.lbtNotice.Text = GetNotice();
                    //bNoticeOk = true;
                //}
            }
            if (!bNoticeOk)
			{
				if (dgvCell.Tag == null)
				{
					...
					{
						if (dgvCell.Value.ToString() != "")
							dgvCell.Tag = Voucher.GetTenHd(dgvCell.Value.ToString());//DataTool.SQLGetNameByCode("R81DmHd", "Ma_Hd", "Ten_Hd", dgvCell.Value.ToString());
					}
		...
		void dgvEditCt_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
		{
			...
			if (this.ActiveControl == dgvEditCt || this.ActiveControl.GetType().Name == "DataGridViewTextBoxEditingControl")
			{
				...
				if (Common.Inlist(strColumnName, "TK_NO,TK_CO,TK_NO3,TK_CO3"))
					...
					bLookup = dgvLookupMa_Dt(ref dgvCell, strColumnName);

				else if (Common.Inlist(strColumnName, "MA_DT_CBNV"))
				...
				else if (Common.Inlist(strColumnName, "MA_VT_SP,MA_VT_SP_CO"))
		...
		private bool dgvLookupMa_Hd(ref DataGridViewCell dgvCell)
		{
			...
			else
			{
				...
				dgvCell.Tag = drLookup["So_Hd"].ToString();
		...
		private bool dgvLookupMa_Dt(ref DataGridViewCell dgvCell, string strColumnName)
		{
			string strValue = string.Empty;
			...
			//if (dgvCell.OwningColumn.Name.ToUpper() == "MA_DT_CO")
			//	bRequire = false;
			drCurrent = ((DataRowView)bdsEditCt.Current).Row;
			DataRow drLookup = Lookup.ShowLookup("Ma_Dt", strValue, bRequire, "", "");
			...
			else
			{
				...
				if (strColumnName == "MA_DT_CO" && drCurrent["Ma_Dt"].ToString() == string.Empty && drCurrent["Ma_Dt_Co"].ToString() != string.Empty)
					drCurrent["Ong_Ba"] = drLookup["Ten_Dt"].ToString();
				else if (strColumnName == "MA_DT" && drCurrent["Ma_Dt"].ToString() != string.Empty && drCurrent["Ma_Dt_Co"].ToString() == string.Empty)
					drCurrent["Ong_Ba"] = drLookup["Ten_Dt"].ToString();
			}
			return true;
		}
		...
		private bool dgvLookupMa_Km(ref DataGridViewCell dgvCell)
		{
			...
			if ((bool)SQLExec.ExecuteReturnValue("SELECT Tk_Km FROM R81DMTK WHERE Tk = '" + strTk_No + "'") && strTk_No != string.Empty)
				bRequire = true;
			else if ((bool)SQLExec.ExecuteReturnValue("SELECT Tk_Km FROM R81DMTK WHERE Tk = '" + strTk_Co + "'") && strTk_Co != string.Empty)
				bRequire = true;
		...
		private bool dgvLookupMa_Vt_Sp(ref DataGridViewCell dgvCell)
		{
			...
			if (strTk_No != string.Empty && (bool)SQLExec.ExecuteReturnValue("SELECT Tk_Sp FROM R81DMTK WHERE Tk = '" + strTk_No + "'"))
				bRequire = true;
			else if (strTk_Co != string.Empty && (bool)SQLExec.ExecuteReturnValue("SELECT Tk_Sp FROM R81DMTK WHERE Tk = '" + strTk_Co + "'"))
				bRequire = true;
		...
		private bool dgvLookupMa_Thue(ref DataGridViewCell dgvCell)
		{
			...
			this.Ma_Thue_Valid();
			//lấy MST
			string strMa_Dt = drCurrent["Ma_Dt_Co"].ToString();
			if (drCurrent["Ma_Dt"].ToString() != "" && drCurrent["Ma_Dt_Co"].ToString() == "")
			...
			{ 
				...
				if(drDmDt != null) 
				{
					drCurrent["Ma_So_Thue"] = drDmDt["Ma_So_Thue"];
					drCurrent["Ten_DtGtGt"] = drDmDt["Ten_Dt"];
				}
			}
		...
		private bool dgvLookupMa_So_Thue(ref DataGridViewCell dgvCell)
		{
			...
			else if (strValue != string.Empty && (drCurrent["Ma_So_Thue", DataRowVersion.Original] == DBNull.Value 
						|| strValue != (string)drCurrent["Ma_So_Thue", DataRowVersion.Original]))
			{
				DataTable dtLookup = SQLExec.ExecuteReturnDt("SELECT * FROM vw_Ma_So_Thue WHERE Ma_So_Thue = '" + strValue + "'");
				...
				if (dtLookup != null)
				{
					if (dtLookup.Rows.Count == 1)
					{
						...
						dgvCell.Tag = dtLookup.Rows[0]["Ten_DtGtGt"].ToString();

						drCurrent["Ma_So_Thue"] = dgvCell.Value;
						drCurrent["Ten_DtGtGt"] = dtLookup.Rows[0]["Ten_DtGtGt"].ToString(); //dgvCell.Tag;
					}
					else
					{
