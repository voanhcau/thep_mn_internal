...
namespace RosyModule.Inventory
{
    public partial class frmCtNX_Edit : frmVoucher_Edit
    {
        ...
        public frmCtNX_Edit()
        {
            ...
            this.btPhanBo_SLCan.Click += new EventHandler(btPhanBo_SLCan_Click);
        ...
        void dgvEditCt1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                //if (Convert.ToDouble(drCurrent["Barem"]) == 0 && Common.Inlist(strColumn_Name, "IS_BAREM") && !Common.Inlist(drCurrent["Ma_Ct"].ToString(),"PXKV,PXCP"))
                //{
        ...
        private void Calc_So_Luong_QD(string strColumn_Name, DataRow drCurrent)
        {
            ...
            else
            {
                if (strColumn_Name == "IS_BARCODE")
                {
                    dbSo_Luong9 = drCurrent["So_Luong_Barcode"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["So_Luong_Barcode"]);
                ...
                {
                    dbSo_Luong9 = drCurrent["So_Luong_Barem"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["So_Luong_Barem"]);
                    drCurrent["So_Luong9"] = dbSo_Luong9;
                    drCurrent["IS_BAREM"] = true;
                    drCurrent["IS_80TAN"] = drCurrent["IS_BARCODE"] = false;
                    Voucher.Calc_So_Luong(drCurrent, this);
                ...
                {
                    dbSo_Luong9 = drCurrent["So_Luong_Can"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["So_Luong_Can"]);
                    drCurrent["So_Luong9"] = dbSo_Luong9;
                    drCurrent["IS_80TAN"] = true;
                    drCurrent["IS_BAREM"] = drCurrent["IS_BARCODE"] = false;
        ...
        private void Calc_So_Luong_QD_Old(string strColumn_Name, DataRow drCurrent)
        {
            string strStt_Org = drCurrent["Stt_Org"] == DBNull.Value ? string.Empty : (string)drCurrent["Stt_Org"];
            DataRow drPh_Scale = DataTool.SQLGetDataRowByID("R80PH_SCALE", "Stt", strStt_Org);
            DataRow[] arrEditCt = dtEditCt.Select("Stt_Org = '" + strStt_Org + "'");
            ...
            if (strColumn_Name == "IS_BARCODE")
            {
                if (arrEditCt.Length > 0)
                {
                    foreach (DataRow dr in arrEditCt)
                    {
                        ...
                        dbSo_Luong_Cay = dr["So_Luong_Cay"] == DBNull.Value ? 0 : Convert.ToDouble(dr["So_Luong_Cay"]);
                        dbBarem = dr["Barem"] == DBNull.Value ? 0 : Convert.ToDouble(dr["Barem"]);
                        dbSo_Luong_Can = dr["So_Luong_Barcode"] == DBNull.Value ? 0 : Convert.ToDouble(dr["So_Luong_Barcode"]);
                ...
                else
                {
                    ...
                    dbSo_Luong_Cay = drCurrent["So_Luong_Cay"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["So_Luong_Cay"]);
                    dbBarem = drCurrent["Barem"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["Barem"]);
                    dbSo_Luong_Can = drCurrent["So_Luong_Barcode"] == DBNull.Value ? 0 : Convert.ToDouble(drCurrent["So_Luong_Barcode"]);
            ...
            {
                double dbSo_Luong_80T = 0, dbSo_Luong_LuyKe = 0, dbSo_Luong_CL = 0;

                if (drPh_Scale != null)
                    dbSo_Luong_80T = drPh_Scale["So_Luong"] == DBNull.Value ? 0 : Convert.ToDouble(drPh_Scale["So_Luong"]);
                ...
                if (arrEditCt.Length > 0)
                {
                    foreach (DataRow dr in arrEditCt)
                    {
                        ...
                        dbSo_Luong9 = dbSo_Luong_Cay * dbBarem;

                        dbSo_Luong_LuyKe += dbSo_Luong9;
                    ...
                    dbSo_Luong_CL = dbSo_Luong_LuyKe - dbSo_Luong_80T;
                    if (drCurrent.Table.Columns.Contains("So_Luong_CL"))
                    {
                        drCurrent["So_Luong_CL"] = dbSo_Luong_CL;

                        if (dbSo_Luong_CL < 0)
                            drCurrent["Is_Print_Barem"] = true;
                        else
                            drCurrent["Is_Print_Barem"] = false;
                    }
                }
            }
            else if (strColumn_Name == "IS_80TAN")
            {
                double dbSo_Luong_80T = 0, dbSo_Luong_LuyKe = 0, dbSo_Luong_Max = 0, dbSo_Luong_CL = 0;
                int iStt0_Max = 0;
                ...
                if (arrEditCt.Length > 0)
                {
                    foreach (DataRow dr in arrEditCt)
                    {
                        ...
                        /////
                        if (dbSo_Luong9 > dbSo_Luong_Max)
                        {
                            dbSo_Luong_Max = dbSo_Luong9;
                            //Them moi thi Ident00 bang 0. Khong lay duoc
                            //iIdent00_Max = Convert.ToInt32(dr["Ident00"]);
                            iStt0_Max = Convert.ToInt32(dr["Stt0"]);
                        }
                        dbSo_Luong_LuyKe += dbSo_Luong9;
                        ////
                    ...
                    if (iStt0_Max > 0) //Điều chỉnh chênh lệch vào trường So_Luong_Max
                    {
                        foreach (DataRow dr in arrEditCt)
                        {
                            if (Convert.ToInt32(dr["Stt0"]) == iStt0_Max)
                                dr["So_Luong9"] = Convert.ToDouble(dr["So_Luong9"]) + dbSo_Luong_80T - dbSo_Luong_LuyKe;
                        }
                    }
        ...
        private void Build()
        {
            ...
            if (!Common.Inlist(strMa_Ct, "PX,PXHCP,PXDCA,PXB,PXBR,PXDC,PXKV"))
            {
                txtSo_Xe.Visible = false;
                rsLabel3.Visible = false;
                txtSo_Xa_Lan_Tau.Visible = false;
                rsLabel5.Visible = false;
            }
        ...
        private void Init_Ct()
        {
            ...
            if (this.enuNew_Edit == enuEdit.New || this.enuNew_Edit == enuEdit.Copy)
            {
                ...
                {
                    txtSo_Ct_Barem.Visible = true;
                    lbtSo_Ct_Barem.Visible = true;
            ...
            if (Common.Inlist(strMa_Ct, "PXCP,PXBR,PXDC"))
            {
                txtSo_Ct_Barem.Visible = true;
                lbtSo_Ct_Barem.Visible = true;
                ...
                {
                    numTSo_Luong_Can.Visible = true;
                    btPhanBo_SLCan.Visible = true;
                    lbtTSo_Luong_Can.Visible = true;
                    txtHt_Gn.Visible = true;
                    lbtHt_Gn.Visible = true;
                    lbtTen_Ht_Gn.Visible = true;
                  
                    lbtSo_Ct_Barem.Text = "Số phiếu cân";
                }
        ...
        private void LoadDicName()
        {
            ...
            {
                lbtHt_Gn.Text = DataTool.SQLGetNameByCode("R81DMTYPE", "Type_ID", "Type_Name", txtHt_Gn.Text.Trim(), "TYPE = 'HT_GN'");
            }
            else
                lbtHt_Gn.Text = string.Empty;
        ...
        private bool FormCheckValid()
        {
            ...
            string strSo_Seri0 = DataTool.SQLGetNameByCode("R81DMNVU", "Ma_Nvu", "So_Seri0", txtMa_Nvu.Text);
            if ((txtSo_Seri0.Text == "") && strMa_Ct == "PXDC" && strSo_Seri0 != "")
            {
            ...
            foreach (DataRow dr in dtEditCt.Rows)
            {
                ...
                if (txtMa_Nvu.Text == "AP22" && dr["Tk_No"].ToString() == dr["Tk_Co"].ToString() && dr["Ht_Gn"].ToString().StartsWith("KG"))
                {
                    string strMsg = string.Empty;
                ...
                if (txtMa_Nvu.Text == "AP22" && dr["Tk_No"].ToString() != dr["Tk_Co"].ToString() && dr["Ht_Gn"].ToString().StartsWith("DD") 
                        && !Common.InlistLike(txtMa_KhoN.Text,"058,055"))
                {
                    string strMsg = string.Empty;
                ...
                if (strMa_Ct == "PXDC" && txtMa_Nvu.Text == "AP22" && txtSo_LXH.Text != "")
                {
                    string strMa_Kho_LXH = DataTool.SQLGetNameByCode("R04CTSO", "So_Ct", "Ma_Kho", txtSo_LXH.Text);
                    if(dr["Ma_Kho"].ToString() != strMa_Kho_LXH)
                    {
                        string strMsg = "Vui lòng kiểm tra mã kho " + dr["Ma_Kho"] + " tại dòng vật tư " + dr["Ten_Vt"] + " khác với lệnh xuất hàng "+ strMa_Kho_LXH + "!!!";

                        Common.MsgCancel(strMsg);
                        return false;
                    }
                }
                if (strMa_Ct.StartsWith("PX") && strMa_Ct != "PXCP" && dr["Ht_Gn"].ToString() == "KG")
                {
                ...
                //kiểm tra hợp lệ của bó cây
                Hashtable htKho = new Hashtable();
                htKho.Add("MA_VT", dr["Ma_Vt"]);
                htKho.Add("SO_LUONG_BO", dr["So_Luong_Bo"]);
                htKho.Add("SO_LUONG_CAY_LE", dr["So_Luong_Cay_Le"]);
                
                if (dr["Ma_Vt"].ToString().StartsWith("BD") && !dr["Ma_Vt"].ToString().StartsWith("BD000000") && !dr["Ma_Vt"].ToString().Contains("1200")
                    && !dr["Ma_Vt"].ToString().Contains("0000")  && Convert.ToDouble(dr["So_Luong"]) 
                        - Convert.ToDouble(SQLExec.ExecuteReturnValue("SELECT dbo.fn_CalBaremBo(@Ma_Vt,@So_Luong_Bo,@So_Luong_Cay_Le)", htKho,CommandType.Text)) > 10) // lệch 10kg
                {
                    string strMsg = string.Empty;

                    strMsg = "Vui lòng nhập số bó hoặc cây lẻ nếu là thép cây  tại dòng " + dr["Ten_Vt"];
        ...
        public override bool Save()
        {
            ...
            if((strMa_Ct == "PXBR" && txtMa_Nvu.Text == "PX14") || (strMa_Ct == "PXKV" && dtEditCt.Rows[0]["Ht_Gn"].ToString() == "GK"))
            {
        ...
        private void InheritVoucher() //Kế thừa chứng từ từ phiếu khác
        {
            ...
            else if (Common.Inlist(strMa_Ct, "PXBR,PXKV,PXDC,PXDCA,PXCP,PXHCP,PXDCC,PXDCD,PNGK,PXGK,PXKG,PXB,PNCP"))
            {
                frmInheritVoucher frm = new frmInheritVoucher();
                frm.Load(this);
        ...
        void btInherit_Click(object sender, EventArgs e)
        {
            ...
            if (txtSo_Ct.Text.IndexOf("NA") <= 0)
                this.txtMa_Nvu_Validating(null, null);
        }

        void btPhanBo_SLCan_Click(object sender, EventArgs e)
        {
            double dbTSo_Luong_Can = numTSo_Luong_Can.Value;
            double dbSo_Luong_Barem = Common.SumDCValue(dtEditCt, "So_Luong_Barem", "");
            ...
            foreach (DataRow dr in dtEditCt.Rows)
            {
                ...
                //dr["So_Luong_Can"] = Math.Round((Convert.ToDouble(dr["So_Luong_Barem"]) / dbSo_Luong_Barem) * dbTSo_Luong_Can, MidpointRounding.AwayFromZero);
                dr["So_Luong"] = dr["So_Luong9"] = Math.Round((Convert.ToDouble(dr["So_Luong_Barem"]) / dbSo_Luong_Barem) * dbTSo_Luong_Can, MidpointRounding.AwayFromZero);
            }

            double dbSo_Luong_Can = Common.SumDCValue(dtEditCt, "So_Luong", "");
            
            if (dbSo_Luong_Can != dbTSo_Luong_Can)
            {
                int iStt0 = Convert.ToInt32(Common.MaxDCValue(dtEditCt, "So_Luong"));
                foreach (DataRow dr in dtEditCt.Select("Stt0 = " + iStt0 + ""))
                {
                    dr["So_Luong"] = dr["So_Luong9"] = Convert.ToDouble(dr["So_Luong"]) + (dbTSo_Luong_Can - dbSo_Luong_Can);
                }
            }
        ...
        {
            if (this.ActiveControl == dteNgay_Ct && strMa_Ct == "PXDC")
                dteNgay_Ct0.Text = dteNgay_Ct.Text;
        }
        void txtSo_Ct_TextChanged(object sender, EventArgs e)
        {
            if (this.ActiveControl == txtSo_Ct && strMa_Ct == "PXDC")
                txtSo_Ct0.Text = txtSo_Ct.Text;
        ...
        void dgvEditCt_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (this.ActiveControl == dgvEditCt || this.ActiveControl.GetType().Name == "DataGridViewTextBoxEditingControl")
            {
                ...
                else if (strColumnName == "LOAI_SP")
                    bLookup = dgvLookupLoai_Sp(ref dgvCell);
        ...
        void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT9,TIEN_NT9,TIEN"))
            {
                ...
                if ((string)drCurrent["Ma_Vt"] != string.Empty && (string)drCurrent["Ma_Kho"] != string.Empty &&
                    (string)drDmCt["Nh_Ct"] == "2" && strColumnName == "SO_LUONG9" && !Common.Inlist(strMa_Ct, "PXCP,PXDCC"))
                {
            ...
            if (Common.Inlist(strMa_Ct, "PXBR,PXKV,PXDC,PXDCC,PXCP,PN"))
            {
                if(!Common.InlistLike(drCurrent["Ma_Kho"].ToString(), "551"))
                { 
                    if (!Common.InlistLike(drCurrent["Stt_Org"].ToString(), "A018"))
                    {
                        if (Common.Inlist(strColumnName, "SO_LUONG_BO,SO_LUONG_CAY_LE,SO_LUONG_CAY,BAREM,SO_LUONG9"))
                        {
                            ...
                            drCurrent["So_Luong_Barem"] = SQLExec.ExecuteReturnValue("SELECT dbo.fn_CalBaremBo(@Ma_Vt,@So_Bo,@So_Cay_Le)", ht, CommandType.Text);
                            if (Convert.ToDouble(drCurrent["So_Luong_Barem"]) != 0 && numTSo_Luong_Can.Value == 0)
                                drCurrent["So_Luong9"] = drCurrent["So_Luong"] = drCurrent["So_Luong_Barem"];
                            else // la cuon
                ...
                else if (Common.InlistLike(drCurrent["Ma_Kho"].ToString(), "551"))
                    drCurrent["So_Luong"] = drCurrent["So_Luong9"];
            }
        ...
        private bool dgvLookupLoai_Sp(ref DataGridViewCell dgvCell)
        {
            string strValue = string.Empty;
            ...
            System.Collections.Hashtable htField = new System.Collections.Hashtable();
            htField.Add("strType", "LOAI_SP");
            DataRow drLookup = Lookup.ShowLookup("Type_ID", strValue, bRequire, "TYPE = 'LOAI_SP'", "", htField);
