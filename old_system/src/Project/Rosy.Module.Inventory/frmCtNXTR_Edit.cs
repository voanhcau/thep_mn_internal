...
{
	public partial class frmCtNXTR_Edit : frmVoucher_Edit
	{
		...
		public frmCtNXTR_Edit()
		{
			...
			dgvEdit.CellValidated += new DataGridViewCellEventHandler(dgvEdit_CellValidated);
			dgvEdit.CellValidating += new DataGridViewCellValidatingEventHandler(dgvEdit_CellValidating);
			dgvEdit.KeyDown += new KeyEventHandler(dgvEdit_KeyDown);
		...
		void bdsEditCt_PositionChanged(object sender, EventArgs e)
		{
			...
			//bdsEditCt_TR.Filter = "Ma_Vt = '" + drCurrent["Ma_Vt"].ToString() + "' AND So_Me = '" + drCurrent["So_Me"].ToString() + "'";
		}
		...
		private void Build()
		{
			...
			dgvEdit.strZone = (string)drDmCt["Zone_EditCt2"];
		...
		private void FillData()
		{
			...
			this.dgvEdit.DataSource = bdsEditCt;
			dgvEdit.ClearSelection();

			//dgvEditCt2.DataSource = bdsEditCt;
			//dgvEditCt2.ClearSelection();

			//Điền dữ liệu cho R05CTNXLR
			string strSQL = "Select T1.*, Ten_Vt From R05CTNXLR T1 JOIN R81DMVT T2 ON T1.Ma_Vt = T2.Ma_Vt WHERE Stt = '" + strStt + "' ORDER BY Ma_Vt"; //
		
			dtEditCt_LR = SQLExec.ExecuteReturnDt(strSQL);
			DataColumn dc_LR = new DataColumn("Deleted", typeof(bool));
			dc_LR.DefaultValue = false;
			dtEditCt_LR.Columns.Add(dc_LR);

			bdsEditCt_TR.DataSource = dtEditCt_LR;
			dgvEditCt1.DataSource = bdsEditCt_TR;
			
			//Điền dữ liệu cho R05CTNX
			//dtEdit_LR = DataTool.SQLGetDataTable("R05CTNX", "*, CAST ('' AS NVARCHAR(100)) AS Ten_Vt", "Stt = '" + strStt + "'", "Stt0");
			//bdsEdit_TR.DataSource = dtEdit_LR;
			//dgvEditCt1.DataSource = bdsEditCt_TR;
		}
		...
		private void Init_Ct()
		{
			...
			if ((bool)drDmCt["Is_LR"])
			{
				...
				//}
				if (dtEditCt_LR.Rows.Count > 0 && dtEditCt_LR.Rows[0]["Ma_Vt"].ToString() == "")
				{
					dtEditCt_LR.Clear();
				}
				//bdsEditCt.DataSource = dtEditCt_LR;
				//this.d.DataSource = bdsEditCt;
			}
		}
		...
		public override bool Save()
		{
			...
			//Bang: Xóa những dòng không có nhập số liệu
			for (int i = 0; i <= dtEditCt.Rows.Count - 1; i++)
			{

				double dbSo_Luong9 = Convert.ToDouble(dtEditCt.Rows[i]["So_Luong9"]);
				double dbGia_Nt9 = Convert.ToDouble(dtEditCt.Rows[i]["Gia_Nt9"]);
				double dbTien_Nt9 = Convert.ToDouble(dtEditCt.Rows[i]["Tien_Nt9"]);

				if (dbSo_Luong9 + dbGia_Nt9 + dbTien_Nt9 == 0)
					dtEditCt.Rows[i].Delete();

			}
			for (int i = 0; i <= dtEditCt_LR.Rows.Count - 1; i++)
			{

				double dbSo_Luong9 = Convert.ToDouble(dtEditCt_LR.Rows[i]["So_Luong9"]);
				double dbGia_Nt9 = Convert.ToDouble(dtEditCt_LR.Rows[i]["Gia_Nt"]);
				double dbTien_Nt9 = Convert.ToDouble(dtEditCt_LR.Rows[i]["Tien_Nt"]);

				if (dbSo_Luong9 + dbGia_Nt9 + dbTien_Nt9 == 0)
					dtEditCt_LR.Rows[i].Delete();
			}

			foreach (DataRow dr in dtEditCt_LR.Rows)
			{
				dr["Ngay_Ct"] = drEditPh["Ngay_Ct"];
			...
			//return Voucher.SQLUpdateCtTr(this, dtEditCt_LR);
		...
		{//Ham thuc hien phim Enter: true: thuc hien thanh cong, false: khong thuc hien duoc
			...
			{
				drCurrent = ((DataRowView)bdsEditCt_TR.Current).Row;
				...
				if (drCurrent["Ma_Vt"] == DBNull.Value || (string)drCurrent["Ma_Vt"] == string.Empty && dgvCell.OwningRow.Index != 0)
				{
					...
					if (bdsEditCt_TR.Count > 1)
					{
						bdsEditCt_TR.RemoveCurrent();
						//dtEditCt.AcceptChanges();
					}
		...
		{//Ham thuc hien phim Enter: true: thuc hien thanh cong, false: khong thuc hien duoc
			...
			if (Common.Inlist(strCurrentColumn, "TIEN_NT9"))
			{
				...
				if (txtMa_Tte.Text.Trim() == Element.sysMa_Tte)
				{
					 //Cap nhat tien TIEN_NT9 truoc khi xuong dong
					double dbTien_Nt9 = 0;
					if (double.TryParse(dgvEdit.CurrentCell.FormattedValue.ToString().Trim(), out dbTien_Nt9))
					{
						dgvEdit.CancelEdit();
					    drCurrent = ((DataRowView)bdsEditCt.Current).Row;
				...
				case Keys.F8:
					{
						...
						if (bdsEditCt_TR.Position < 0)
							return;

						drEdit_TR = ((DataRowView)bdsEditCt_TR.Current).Row;
						drEdit_TR["Deleted"] = !((bool)drEdit_TR["Deleted"]);
						if ((bool)drEdit_TR["Deleted"] == true)
						{
							Font font = new Font(dgvEditCt1.Font.FontFamily, dgvEditCt1.Font.Size, FontStyle.Strikeout);
							dgvEditCt1.CurrentRow.DefaultCellStyle.Font = font;
						}
						else
						{
							dgvEditCt1.CurrentRow.DefaultCellStyle.Font = dgvEditCt1.Font;
						}
						//dtEditCt_LR.Rows.Remove(drEdit_TR);
					}
					break;
		...
		void dgvEditCt_CellEnter(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (Common.Inlist(strColumnName, "MA_KHO,MA_VT_SP"))
			{
		...
		//Cai dat Lookup, Enter xuống dòng luoi tren
		void dgvEdit_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			dgvVoucher dgvEdit = (dgvVoucher)sender;
			if (this.ActiveControl != dgvEdit && this.ActiveControl != null && this.ActiveControl.GetType().Name != "DataGridViewTextBoxEditingControl")
				return;
		...
		void dgvEditCt_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
		{
			...
			if (this.ActiveControl == dgvEditCt || this.ActiveControl.GetType().Name == "DataGridViewTextBoxEditingControl")
			{
				...
				if (Common.Inlist(strColumnName, "TK_NO,TK_CO"))
					...
					bLookup = dgvLookupMa_Vt_TR(ref dgvCell);

				else if (strColumnName == "MA_KHO")
					bLookup = dgvLookupMa_Kho_TR(ref dgvCell);
		...
		void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT,TIEN_NT,TIEN"))
			{
				//Voucher.Calc_So_Luong(drCurrent, this);
				//Voucher.Update_TTien(this);
			...
			if (Common.Inlist(strColumnName, "DDAI_PHOI,SO_LUONG_CAY"))
			{
				...
				drCurrent["So_Luong9"] = Math.Round(Convert.ToDouble(drCurrent["So_Luong_Cay"]) * Convert.ToDouble(drDmVt["Barem"]) * Convert.ToDouble(drCurrent["DDai_Phoi"]), 0);
				drCurrent["So_Luong"] = Math.Round(Convert.ToDouble(drCurrent["So_Luong_Cay"]) * Convert.ToDouble(drDmVt["Barem"]) * Convert.ToDouble(drCurrent["DDai_Phoi"]), 0);
			}
			bdsEditCt_TR.EndEdit();//Cap nhat lai DataSource
		}
		...
		//Xu ly Dvt o luoi tren
		void dgvEdit_KeyDown(object sender, KeyEventArgs e)
		{
			if (e.KeyCode == Keys.Space)
			{
				if (dgvEdit.CurrentCell.OwningColumn.DataPropertyName == "DVT")
				{
					drCurrent = ((DataRowView)bdsEditCt_TR.Current).Row;
		...
		private bool dgvLookupMa_Vt(ref DataGridViewCell dgvCell)
		{
			...
			else
			{
				...
				{
					//bang them vi khi lookup ko lay ma_vt sang Grid
					drCurrent["Ma_Vt"] = drLookup["Ma_Vt"];
		...
		private bool dgvLookupMa_Vt_TR(ref DataGridViewCell dgvCell)
		{
			string strValue = string.Empty;
		...
		private bool dgvLookupMa_Kho_TR(ref DataGridViewCell dgvCell)
		{
			string strValue = string.Empty;
