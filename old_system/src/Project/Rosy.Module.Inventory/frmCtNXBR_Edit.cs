...
{
	public partial class frmCtNXBR_Edit : frmVoucher_Edit
	{
		...
		public frmCtNXBR_Edit()
		{
           ...
           // this.btImportAccess.Click += new EventHandler(btImportAccess_Click);
		...
		private void Calc_So_Luong_QD(string strColumn_Name, DataRow drCurrent)
		{
			...
			{
				double dbSo_Luong_80T = 0, dbSo_Luong_LuyKe = 0, dbSo_Luong_Max = 0;
				int iIdent00_Max = 0;
				...
				if (arrEditCt.Length > 0)
				{
					...
					if (iIdent00_Max != 0) //Điều chỉnh chênh lệch vào trường So_Luong_Max
					{
						foreach (DataRow dr in arrEditCt)
						{
							if (Convert.ToInt32(dr["Ident00"]) == iIdent00_Max)
							{
								dr["So_Luong9"] = (dbSo_Luong_80T - dbSo_Luong_LuyKe);
							}
						}
					}
				}
		...
		private void Ma_Tte_Valid()
		{
            ...
            if (strMa_Ct == "PNSB" || strMa_Ct == "PXSB")
            {
                if (dgvEditCt1.Columns.Contains("So_Luong"))
                    dgvEditCt1.Columns["So_Luong"].DefaultCellStyle.Format = "N3";
                if (dgvEditCt1.Columns.Contains("So_Luong9"))
                    dgvEditCt1.Columns["So_Luong9"].DefaultCellStyle.Format = "N3";
            }
		...
		private void InheritVoucher() //Kế thừa chứng từ từ phiếu khác
		{
			...
			if (Common.Inlist(strMa_Ct, "PNPT,PXPT"))
			{
			...
			else if (Common.Inlist(strMa_Ct, "PXBR"))
			{
				frmInheritVoucher frm = new frmInheritVoucher();
				frm.Load(this);
        ...
        private void ImportAccess() //Kế thừa chứng từ từ file Access
        {
            Voucher.Update_Header(this);
            Voucher.Update_Detail(this);

            frmImportAccess frm = new frmImportAccess();
            frm.Load(this.drEditPh);

            if (frm.Is_Accept)
            {
                Voucher.ImportAccess_SetData(frm, this);
        ...
        void btImportAccess_Click(object sender, EventArgs e)
        {
            this.ImportAccess();
        }
	...
	//    }
		...
		void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
            ...
            if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT9,TIEN_NT9,TIEN"))
            {
                ...
                if (Common.Inlist(strColumnName, "DDAI_PHOI") && (strMa_Ct == "PNSB" || strMa_Ct == "PXSB"))
                {
                    double dbDDai_Phoi_Old = 0;
                    int iSo_Cay = Convert.ToInt16(drCurrent["So_Luong_Cay"]);
                    int iSo_Cay_Old = 0;
                    
                    if (drCurrent.HasVersion(DataRowVersion.Original))
                    {
                        dbDDai_Phoi_Old = Convert.ToDouble(drCurrent["DDAI_PHOI", DataRowVersion.Original]) == 12 ? 12 : Convert.ToDouble(drCurrent["DDAI_PHOI", DataRowVersion.Original]);
                        iSo_Cay_Old = Convert.ToInt16(drCurrent["So_Luong_Cay", DataRowVersion.Original]);
                    }

                    if (drCurrent["DDai_Phoi"].ToString() == "6" && dbDDai_Phoi_Old == 12 && iSo_Cay_Old == iSo_Cay && drCurrent["Loai_Phoi"].ToString() == "120x120")
                        drCurrent["So_Luong_Cay"] = iSo_Cay * 2;
                    else
                    if (drCurrent["DDai_Phoi"].ToString() == "12" && dbDDai_Phoi_Old == 6 && iSo_Cay_Old == iSo_Cay && drCurrent["Loai_Phoi"].ToString() == "120x120")
                        drCurrent["So_Luong_Cay"] = iSo_Cay/2;
                }

                if (Common.InlistLike(strMa_Ct, "PNSB, PXSB"))
                {
                    ...
                    drCurrent["So_Luong9"] = Convert.ToDouble(drCurrent["So_Luong_Cay"]) * Convert.ToDouble(drDmVt["Barem"]) * Convert.ToDouble(drCurrent["DDai_Phoi"])/12;
                }
