...
{
	public partial class frmCtNXPT_Edit : frmVoucher_Edit
	{
		...
		private bool FormCheckValid()
		{
            ...
            {
                string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Chưa nhập mã bộ phận" : "Do not register transaction type";
                Common.MsgCancel(strMsg);
                return false;
            }
            if (txtMa_Dt_CbNv.Text == string.Empty)
            {
                string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Chưa nhập mã nhân viên" : "Do not register transaction type";
                Common.MsgCancel(strMsg);
                return false;
            }
			...
			foreach (DataRow dr in dtEditCt.Rows)
			{
                ...
                if(dr["Ma_VTri"] == "")
                {
                    string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Chưa nhập mã vị trí tại mã vật tư "+ dr["Ma_Vt"] +"" : "Do not register transaction type";
                    Common.MsgCancel(strMsg);
                    return false;
                }
		...
		private void Ma_Tte_Valid()
		{
			...
			else
			{
				...
				if (dteNgay_Ct.Text != Library.DateToStr((DateTime)drEditPh["Ngay_Ct"]) || txtMa_Tte.bTextChange || txtMa_Nvu.bTextChange)
				{
		...
		private void CheckViTri() //Kế thừa chứng từ từ phiếu khác
		{
			drCurrent = ((DataRowView)bdsEditCt.Current).Row;

			frmCheckViTri frm = new frmCheckViTri();
			frm.Load(drCurrent);

			if (frm.drLookup != null)
			{
				drCurrent["Ma_Kho"] = frm.drLookup["Ma_Kho"];
				drCurrent["Phong"] = frm.drLookup["Phong"];
				drCurrent["Ke"] = frm.drLookup["Ke"];
				drCurrent["Tang"] = frm.drLookup["Tang"];
				drCurrent["O"] = frm.drLookup["O"];
				
				if(strMa_Ct == "PXPT")
					drCurrent["So_Luong9"] = frm.drLookup["Ton_Cuoi"];
			}
		}
		...
		void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT9,TIEN_NT9,TIEN"))
			{
				...
				if (enuNew_Edit == enuEdit.New || enuNew_Edit == enuEdit.Copy)
				{
					if ((bool)drCurrent["Auto_Cost"] && drCurrent["Ma_Vt"].ToString() != "" && drCurrent["Ma_Kho"].ToString() != "" && Convert.ToDouble(drCurrent["So_Luong"]) != 0)
					{
						...
						double dbTien_TT = Convert.ToDouble(SQLExec.ExecuteReturnValue("SELECT dbo.fn_GetTien_TT_Pt(@Ngay_Ct, @Ma_Kho, @Ma_Vt, @Stt, @Ma_DvCs)", htPara, CommandType.Text));
						double dbSL_TT = Convert.ToDouble(SQLExec.ExecuteReturnValue("SELECT dbo.fn_GetSL_TT_Pt(@Ngay_Ct, @Ma_Kho, @Ma_Vt, @Stt, @Ma_DvCs)", htPara, CommandType.Text));
            ...
            if (Common.Inlist(strColumnName, "SO_LUONG9") && strMa_Ct == "PNPT")
            {

                if (Convert.ToDouble(drCurrent["So_Luong9"]) > 10)
                    drCurrent["So_Lan_In"] = 10;
                else
                    drCurrent["So_Lan_In"] = drCurrent["So_Luong9"];
