...
{
	public partial class frmInherit_CtSO_Scale : RosySystem.Customize.frmView
	{
		#region Declare
        ...
        public string strMa_Nvu = string.Empty;
		...
		void Build()
		{
			dgvInheritVoucher.strZone = "INHERIT_LXH_SCALE";
		...
		void FillData()
		{
			if (!string.IsNullOrEmpty(this.strStt_Org))
			{
                string strSQLExec = "SELECT * FROM R80PH WHERE Ma_Ct LIKE 'LXH%' AND Stt = '" + this.strStt_Org + "' ";//AND Duyet_Huy = 0
            ...
            string strQuery = @"
					SELECT T1.*, Ten_Vt, CAST(0 AS BIT) AS Chon 
						FROM " + strTableCt + @" T1 LEFT JOIN (SELECT Ma_Vt AS Ma_Vt_Kh, Ten_Vt FROM R81DMVT) T2 ON T1.Ma_Vt = T2.Ma_Vt_Kh
						WHERE " + @strKey + @"
						ORDER BY Stt,Stt0";
            ...
            //HT.Add("STT", txtStt);
            //dtInheritVoucher = SQLExec.ExecuteReturnDt("sp_GetCTSO_Scale", HT, CommandType.StoredProcedure);
			...
			if (dtInheritVoucher.Rows.Count > 0)
			{
                ...
                if (dtInheritVoucher.Rows[0]["Ma_Nvu"].ToString() == "LXH02")//XỬ LÝ CHO HÀNG GỮI KHO ĐI BẺ
                {
                    strMa_Dt_TTKH = txtMa_Dt.Text = "1000043";
                    strMa_Nvu = "LXH02";
                    strSo_Xe_TTKH = txtSo_Xe.Text = SQLExec.ExecuteReturnValue("SELECT [dbo].[fn_GetSoXeFromMaXe] ('" + dtInheritVoucher.Rows[0]["Ma_Xe"].ToString() + "')").ToString();
                }
                else
		...
		void txtStt_Search_KeyDown(object sender, KeyEventArgs e)
		{
			if (e.KeyCode == Keys.Return)
			{
                ...
                string strSQLExec = "SELECT * FROM R80PH WHERE Ma_Ct LIKE 'LXH%' AND Stt = '" + txtStt_Search.Text.Trim() + "' "; //AND Duyet_Huy = 0
				DataTable dtPh = SQLExec.ExecuteReturnDt(strSQLExec, CommandType.Text);
				if (dtPh.Rows.Count > 0)
				{
					DataRow drPh = dtPh.Rows[0];
