...
{
	public partial class frmXuatVTriKho : RosySystem.Customize.frmView
	{
		#region Declare
		...
		public void Load(DataRow drEdit_Ct)
		{
			strStt = drEdit_Ct["Stt"].ToString();
			strMa_Ct = drEdit_Ct["Ma_Ct"].ToString();
			dtNgay_Ct = Convert.ToDateTime(drEdit_Ct["Ngay_Ct"]);
			Build();
			FillData();
			BindingLanguage();
		...
		void Build()
		{
			dgvInheritVoucher.strZone = "DNX_EDITCT4";
			...
			if (dgvInheritVoucher.Columns.Contains("SO_LUONG"))
				dgvInheritVoucher.Columns["SO_LUONG"].ReadOnly = false;
            if (dgvInheritVoucher.Columns.Contains("GHI_CHU"))
                dgvInheritVoucher.Columns["GHI_CHU"].ReadOnly = false;
		}
		...
		bool Save()
		{
			...
			DataTable dtImport = dtXuatVTriKho.Clone();

			DataRow[] Result = dtXuatVTriKho.Select("CHON = 1");
            ...
            sqlCom.Parameters.Add("STT", strStt);
			sqlCom.Parameters.Add("MA_DVCS", Element.sysMa_DvCs);

			sqlCom.CommandText = "Sp_ImportTVP_CTNXVTRI";

			SqlParameter sqlPara = new SqlParameter();
			sqlPara.ParameterName = "@TVP_Import";
			sqlPara.SqlDbType = SqlDbType.Structured;
			sqlPara.TypeName = "TVP_CTNXVTRI";
			sqlPara.Value = Voucher.GetTVPValue("R05CTNXVTRI", "TVP_CTNXVTRI", dtImport);
			...
			try
			{
                ...
                SQLExec.Execute("sp_Import_VTPT_KPH", ht, CommandType.StoredProcedure);
