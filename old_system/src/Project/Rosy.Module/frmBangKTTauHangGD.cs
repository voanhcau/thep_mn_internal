...
{
    public partial class frmBangKTTauHangGD : RosySystem.Customize.frmView
    {
        DataTable dtKQKN;
     ...
     //   rsDataGridView dgvKQKN = new rsDataGridView();
        ...
        private void Build()
        {
            ...
            dgvKQKN.strZone = "BANGKEBOCHANGGD";
        ...
        {
			string strSQLExec = "SELECT * FROM R02KETQUATAUGD WHERE Stt_Org = '" + strStt + "'";
			...
			{
				txtTg_Cap_Ben.Text = dtKQKN.Rows[0]["Tg_Cap_Ben"].ToString();
				txtTg_Lam_Hang.Text = dtKQKN.Rows[0]["Tg_Lam_Hang"].ToString();
			}
		...
		private void Calc_Tien_All()
		{
			...
			numKL_TC.Value = Math.Round(Common.SumDCValue(dtKQKN, "KL_TC", strKeyFilter), 2, MidpointRounding.AwayFromZero);
		}
		#region Sự kiện
		...
		void dgvKQKN_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (Common.Inlist(strColumnName, "SO_LUONG,TL_HMS1"))
			{
				double dbTong_So_Luong = (drCurrent2["SO_LUONG"] == DBNull.Value) ? 0 : Convert.ToDouble(drCurrent2["SO_LUONG"]);

				double dbTL_HMS1 = (drCurrent2["TL_HMS1"] == DBNull.Value) ? 0 : Convert.ToDouble(drCurrent2["TL_HMS1"]);
				double dbTL_HMS2 = 0;
				double dbKL_HMS1 = 0;
				double dbKL_HMS2 = 0;

				if (dbTong_So_Luong > 0 && dbTL_HMS1 > 0)
				{
					dbTL_HMS2 = 100 - dbTL_HMS1;
					dbKL_HMS1 = Math.Round((dbTL_HMS1 * dbTong_So_Luong) / 100, 2);
					dbKL_HMS2 = dbTong_So_Luong - dbKL_HMS1;
				}

				drCurrent2["TL_HMS2"] = dbTL_HMS2;
				drCurrent2["KL_HMS1"] = dbKL_HMS1;
				drCurrent2["KL_HMS2"] = dbKL_HMS2;

				drCurrent2.AcceptChanges();
				Calc_Tien_All();
			}

			if (Common.Inlist(strColumnName, "TL_TC"))
			{
				double dbTong_So_Luong = (drCurrent2["SO_LUONG"] == DBNull.Value) ? 0 : Convert.ToDouble(drCurrent2["SO_LUONG"]);
				double dbTL_TC = (drCurrent2["TL_TC"] == DBNull.Value) ? 0 : Convert.ToDouble(drCurrent2["TL_TC"]);

				double dbKL_TC = 0;

				if (dbTong_So_Luong > 0 && dbTL_TC > 0)
				{
					dbKL_TC = Math.Round((dbTL_TC * dbTong_So_Luong) / 100, 2);
				}

				drCurrent2["KL_TC"] = dbKL_TC;
		...
		private bool CellKeyEnter()
		{
			...
			if (Common.Inlist(strCurrentColumn, "KL_TC"))
			{
			...
			if (Common.Inlist(strCurrentColumn, "KL_HMS1"))
			{
				...
				if ((Convert.ToDouble(drCurrent["SO_LUONG"]) == 0 && Convert.ToDouble(drCurrent["TL_TC"]) == 0))
				{
		...
		private bool AddRow()
		{
			...
			double db_Tap_Chat = (drCurrent["KL_TC"] == DBNull.Value) ? 0 : Convert.ToDouble(drCurrent["KL_TC"]);

			bool bNewRow;

			if (dbTong_So_Luong + db_KL_HMS1 + db_KL_HMS2 == 0)
				bNewRow = false;
			else
				bNewRow = true;
		...
		{// Update du lieu tu drPh xuong dtCt theo danh sach strColumnList
			...
			foreach (DataRow dr in dtKQKN.Rows)
			{
				...
				dr["Stt_Org"] = strStt;
				dr["Tg_Cap_Ben"] = txtTg_Cap_Ben.Text;
				dr["Tg_Lam_Hang"] = txtTg_Lam_Hang.Text;
		...
		public  bool Save()
		{
			...
			sqlCom.CommandText = "Sp_Update_KETQUATAUGD";
			...
			sqlParaCt.TypeName = "TVP_KETQUATAUGD";
			sqlParaCt.Value = GetTVPValue("R02KETQUATAUGD", "TVP_KETQUATAUGD", dtKQKN);
