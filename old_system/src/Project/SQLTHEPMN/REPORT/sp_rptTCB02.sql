/*
SET DATEFORMAT DMY
EXEC sp_rptTCB02 '20150401','20150430', @Kieu_Nh = '2'
*/
--Bao cao tong hop hang hoa - so xe va khach hang - so xe
ALTER PROCEDURE sp_rptTCB02
(
	...
	@So_Ct VARCHAR(20) = '',
	@So_Xe VARCHAR(20) = '',
	@Kieu_Nh CHAR(1) = '1',--1-Hang hoa - so xe, 2- Khach hang - so xe
...
BEGIN
	...
	IF @Kieu_Nh = '1' --Nhom theo Ma san pham - so xe
	BEGIN
		SELECT	Ma_Vt_Sp, So_Ct, MAX(Ten_Vt_Sp) AS Ten_Vt_Sp, So_Xe,
				MAX(Ma_Dt) AS Ma_Dt, MAX(Ten_Dt) AS Ten_Dt,
				SUM(So_Luong_Vao) AS So_Luong_Vao,
				SUM(So_Luong_Ra) AS So_Luong_Ra,
				SUM(So_Luong) AS So_Luong,
				MAX(Stt_Sx) AS Stt_Sx,
				CAST('' AS VARCHAR(20)) AS Ma_Tg4
			INTO #T_PhatSinhSum
			FROM #T_PhatSinh
			GROUP BY Ma_Vt_Sp, So_Ct, So_Xe
		
		CREATE INDEX Ma_Vt_Sp ON #T_PhatSinhSum(Ma_Vt_Sp)
		CREATE INDEX So_Xe ON #T_PhatSinhSum(So_Xe)
		...
		UPDATE T1 SET So_Xe = T2.So_Xe
			FROM #T_BaoCao T1 JOIN (SELECT Ma_Vt_Sp, COUNT(DISTINCT So_Xe) AS So_Xe FROM #T_BaoCao WHERE Bold = 0 GROUP BY Ma_Vt_Sp) T2 ON T1.Ma_Vt_Sp = T2.Ma_Vt_Sp
			WHERE T1.Bold = 1

		UPDATE #T_BaoCao SET Stt_Sx = T2.Stt
			FROM #T_BaoCao T1 JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY Ma_Vt_Sp ORDER BY SortPath, Level, Ma_Vt_Sp, So_Ct, So_Xe) AS Stt, Ma_Vt_Sp, So_Ct, So_Xe FROM #T_BaoCao WHERE Bold = 0) T2 ON T1.Ma_Vt_Sp = T2.Ma_Vt_Sp AND T1.So_Ct = T2.So_Ct AND T1.So_Xe = T2.So_Xe
			WHERE Bold = 0
	...
	ELSE IF @Kieu_Nh = '2' --Nhom theo Ma doi tuong - so xe
	BEGIN
		SELECT	Ma_Dt, So_Ct, MAX(Ten_Dt) AS Ten_Dt, So_Xe,
				MAX(Ma_Vt_Sp) AS Ma_Vt_Sp, MAX(Ten_Vt_Sp) AS Ten_Vt_Sp,
				SUM(So_Luong_Vao) AS So_Luong_Vao,
				SUM(So_Luong_Ra) AS So_Luong_Ra,
				SUM(So_Luong) AS So_Luong,
				MAX(Stt_Sx) AS Stt_Sx
			INTO #T_PhatSinhSum0
			FROM #T_PhatSinh
			GROUP BY Ma_Dt, So_Ct, So_Xe
		
		
		CREATE INDEX Ma_Dt ON #T_PhatSinhSum0(Ma_Dt)
		CREATE INDEX So_Ct ON #T_PhatSinhSum0(So_Ct)
		CREATE INDEX So_Xe ON #T_PhatSinhSum0(So_Xe)
		...
		UPDATE T1 SET So_Xe = T2.So_Xe
			FROM #T_BaoCao T1 JOIN (SELECT Ma_Dt, COUNT(DISTINCT So_Xe) AS So_Xe FROM #T_BaoCao WHERE Bold = 0 GROUP BY Ma_Dt) T2 ON T1.Ma_Dt = T2.Ma_Dt
			WHERE T1.Bold = 1

		UPDATE #T_BaoCao SET Stt_Sx = T2.Stt
			FROM #T_BaoCao T1 JOIN (SELECT ROW_NUMBER() OVER (PARTITION BY Ma_Dt ORDER BY SortPath, Level, Ma_Dt, So_Ct, So_Xe) AS Stt, Ma_Dt, So_Ct, So_Xe FROM #T_BaoCao WHERE Bold = 0) T2 ON T1.Ma_Dt = T2.Ma_Dt AND T1.So_Ct = T2.So_Ct AND T1.So_Xe = T2.So_Xe
			WHERE Bold = 0
		
		INSERT INTO #T_BaoCao(Ten_Dt, So_Xe, So_Luong_Vao, So_Luong_Ra, So_Luong, SortPath, Bold)
		SELECT dbo.fn_GetLanguage('Tong_Cong', @Language_Type), CONVERT(VARCHAR, ISNULL(COUNT(DISTINCT So_Xe), 0)), SUM(So_Luong_Vao), SUM(So_Luong_Ra), SUM(So_Luong), MAX(SortPath) + 1, 1
			FROM #T_BaoCao
			WHERE Bold = 0
