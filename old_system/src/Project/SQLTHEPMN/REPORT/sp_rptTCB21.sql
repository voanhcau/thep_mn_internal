...
BEGIN
	...
	IF (@Kieu_Nhom = '1')
	BEGIN
		INSERT INTO #T_BaoCao (Ca, Ten_Vt_Sp, So_Bo, So_Cay, So_Luong, So_Luong_Barem, Level, Bold)
			SELECT Ca, 'Total Ca ' + Ca, SUM(So_Bo), SUM(So_Cay), SUM(So_Luong), SUM(So_Luong_Barem), 2, 1
				FROM #T_BaoCao
				GROUP BY Ca
	END	

	INSERT INTO #T_BaoCao (Ca, Ten_Vt_Sp, So_Bo, So_Cay, So_Luong, So_Luong_Barem, Level, Bold)
		SELECT 'Z', N'Tổng cộng', SUM(So_Bo), SUM(So_Cay), SUM(So_Luong), SUM(So_Luong_Barem), 1, 1
			FROM #T_BaoCao
			WHERE Bold = 0
	
	UPDATE #T_BaoCao SET So_Luong_CL = So_Luong - So_Luong_Barem, PT_CL = CASE WHEN So_Luong_Barem = 0 THEN 0 ELSE So_Luong * 100/So_Luong_Barem END
...
END
GO
EXEC sp_rptTCB21 '20150628','20150628', ''