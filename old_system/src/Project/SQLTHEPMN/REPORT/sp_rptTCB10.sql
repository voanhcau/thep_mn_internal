/*
SET DATEFORMAT DMY
EXEC sp_rptTCB10 '20150401','20150430', ''
*/
--Báo cáo chênh lệch mã vạch - Barem
ALTER PROCEDURE [dbo].[sp_rptTCB10]
(
...
BEGIN
	...
	INSERT #T_PhatSinh(Stt, Stt_Org, So_Ct_Barem, Ma_Dt, Ma_Vt, So_Xe, So_Xa_Lan_Tau, Num_Bars, So_Luong_Barem, So_Luong_Barcode)
	SELECT Stt, Stt_Org, So_Ct, Ma_Dt, Ma_Vt, So_Xe, So_Xa_Lan_Tau, So_Luong_Cay, So_Luong_Barem, So_Luong_Barcode
		FROM #T_CtBrem
	...
	UPDATE #T_PhatSinh SET So_Luong_CL = So_Luong_Barem - So_Luong_Barcode
	UPDATE #T_PhatSinh SET Pt_CL = (So_Luong_Barem - So_Luong_Barcode) / 100
	...
	IF @_Ma_Nhom <> ''
		EXEC sp_BuildGroup '#T_BaoCao0', '#T_BaoCao', @_Ma_Nhom, 'So_Bo, Num_Bars, So_Luong_Barem, So_Luong_Barcode, So_Luong_CL, PT_CL', @Language_Type, @Ma_DvCs	
	...
	INSERT INTO #T_BaoCao(Ten_Dt, So_Bo, Num_Bars, So_Luong_Barem, So_Luong_Barcode, So_Luong_CL, Pt_CL, Bold)
	SELECT dbo.fn_GetLanguage('Tong_Cong', @Language_Type), ISNULL(SUM(So_Bo), 0), ISNULL(SUM(Num_Bars), 0), ISNULL(SUM(So_Luong_Barem), 0), ISNULL(SUM(So_Luong_Barcode), 0), ISNULL(SUM(So_Luong_CL), 0), ISNULL(SUM(Pt_CL), 0), 1
		FROM #T_BaoCao
		WHERE Bold = 0
	
	SELECT *, So_Luong_Barem AS Trong_Luong_Barem, So_Luong_Barcode AS Trong_Luong_Barcode, So_Luong_CL AS Trong_Luong_CL
		FROM #T_BaoCao
