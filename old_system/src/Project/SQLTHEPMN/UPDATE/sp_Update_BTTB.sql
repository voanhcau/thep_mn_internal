...
/*
DROP PROCEDURE Sp_Update_BTTB

--Tạo lại TVP
EXEC sp_CreateTVPStructure 'R06PH_BTTB', 'TVP_PHBTTB', 1
EXEC sp_CreateTVPStructure 'R06CT_BTTB', 'TVP_CtBTTB', 1
...
*/
IF OBJECT_ID('Sp_Update_BTTB') IS NOT NULL DROP PROCEDURE Sp_Update_BTTB
GO 

CREATE PROCEDURE [dbo].[Sp_Update_BTTB]
(
	@strNew_Edit CHAR(1),
	@PH TVP_PHBTTB READONLY,
	@Ct TVP_CtBTTB READONLY,
	@Stt VARCHAR(15),
	@Ma_Ct VARCHAR(5),
	@Ma_DvCs VARCHAR(5)
)
...
BEGIN
	...
	IF (@strNew_Edit IN ('N','C') AND EXISTS(SELECT Stt FROM R06PH_BTTB WHERE Stt = @Stt))
	BEGIN
	...
	DECLARE @_TablePHName VARCHAR(50) = '', @_TVPPHName VARCHAR(50) = '',
			...
			@_ColumnName VARCHAR(50) = '', @_So_Ct VARCHAR(20) = ''
	...
	SELECT	@_TVPPHName = 'TVP_PHBTTB',
			@_TVPCtName = 'TVP_CtBTTB'
	...
	UPDATE #T_TableCt SET So_Luong = So_Luong9 WHERE So_Luong <>  So_Luong9
	--Bang bo sung khi GD duyet BBHH
	IF(@Ma_Ct = 'BBHH' AND EXISTS(SELECT * FROM #T_TablePH WHERE Duyet_GiamDoc = 1) AND EXISTS(SELECT * FROM #T_TableCt WHERE Is_LyLich = 0))
		UPDATE #T_TableCt SET Is_LyLich = 1
	
	--
	SET @_So_Ct = (SELECT So_Ct FROM #T_TablePH WHERE Stt = @Stt)
	IF (@strNew_Edit IN ('N','C') AND EXISTS(SELECT So_Ct FROM R06PH_BTTB WHERE So_Ct = @_So_Ct))

	BEGIN
		RAISERROR (N'Thêm mới trùng Số Ct [%s]', 16, 1, @_So_Ct)
		RETURN
	END
	BEGIN TRY
		BEGIN TRANSACTION
