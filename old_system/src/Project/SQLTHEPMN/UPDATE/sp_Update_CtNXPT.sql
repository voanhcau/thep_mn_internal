...
/*
DROP PROCEDURE Sp_Update_CtNXPT

--Tạo lại TVP
EXEC sp_CreateTVPStructure 'R80PH', 'TVP_PHNXPT', 1
EXEC sp_CreateTVPStructure 'R05CtNXPT', 'TVP_CtNXPT', 1
...
*/
IF OBJECT_ID('Sp_Update_CtNXPT') IS NOT NULL DROP PROCEDURE Sp_Update_CtNXPT
GO 

CREATE PROCEDURE [dbo].[Sp_Update_CtNXPT]
(
	@strNew_Edit CHAR(1),
	@PH TVP_PHNXPT READONLY,
	@Ct TVP_CtNXPT READONLY,
	@Stt VARCHAR(15),
	@Ma_Ct VARCHAR(5),
	@Ma_DvCs VARCHAR(5)
)
...
BEGIN
	...
	---- Kiểm tra mã vật tư nhập kho có 8 ký tự
	--IF EXISTS(SELECT Stt0 FROM @Ct WHERE LEN(Ma_Vt) < 11 AND Ma_Vt NOT LIKE 'B%' AND Ma_Vt NOT LIKE 'C%')
	--BEGIN
	--	SET @_Stt0_Error = (SELECT MAX(Stt0) FROM @Ct WHERE LEN(Ma_Vt) < 11 AND Ma_Vt NOT LIKE 'B%' AND Ma_Vt NOT LIKE 'C%')
	--	RAISERROR(N'Tồn tại dòng [%d] mã vật tư chưa chính xác. Yêu cầu kiểm tra lại', 16, 1, @_Stt0_Error)
	--	RETURN
	--END
	...
	SELECT	@_TVPPHName = 'TVP_PHNXPT',
			@_TVPCtName = 'TVP_CtNXPT'
