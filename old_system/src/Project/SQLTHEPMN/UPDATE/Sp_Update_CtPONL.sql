...
/****** Object:  StoredProcedure [dbo].[Sp_Update_CtPONL]    Script Date: 10/11/2016 21:24:17 ******/
SET ANSI_NULLS ON
GO
...
/*
DROP PROCEDURE Sp_Update_CtPONL

--Tạo lại TVP
EXEC sp_CreateTVPStructure 'R80PH', 'TVP_PHPONL', 1
EXEC sp_CreateTVPStructure 'R04CtPONL', 'TVP_CtPONL', 1
...
*/
CREATE PROCEDURE [dbo].[Sp_Update_CtPONL]
(
	@strNew_Edit CHAR(1),
	@PH TVP_PHPONL READONLY,
	@Ct TVP_CtPONL READONLY,
	@Stt VARCHAR(15),
	@Ma_Ct VARCHAR(5),
	@Ma_DvCs VARCHAR(5)
)
...
BEGIN
	...
	--IF EXISTS(SELECT Stt0 FROM @Ct WHERE ABS(So_Luong9) <> ABS(So_Luong0))
	--BEGIN
	--	SET @_Stt0_Error = (SELECT MAX(Stt0) FROM @Ct WHERE ABS((So_Luong9 * He_So9) - So_Luong) > 1 AND So_Luong9 <> 0)
		
	--	RAISERROR (N'Tồn tại dòng [%d] cần enter lại cột số lượng yêu cầu', 16, 1, @_Stt0_Error)
	--	RETURN
	--END
	...
	SELECT	@_TVPPHName = 'TVP_PHPONL',
			@_TVPCtName = 'TVP_CtPONL'
