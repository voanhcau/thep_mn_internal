...
/*
DROP PROCEDURE Sp_Update_CtKT

--Tạo lại TVP
EXEC sp_CreateTVPStructure 'R80PH', 'TVP_PHKT', 1
EXEC sp_CreateTVPStructure 'R80CtKT', 'TVP_CtKT', 1

EXEC sp_CreateTVPStructure 'R80HanTt0', 'TVP_HanTt0', 1

--Test thủ tục cập nhật
DECLARE @PH AS TVP_PH, @Ct TVP_CtKT
INSERT INTO @PH SELECT * FROM R80PH WHERE Stt = 'A01010000010064-'
INSERT INTO @Ct SELECT * FROM R80CtKT WHERE Stt = 'A01010000010064-'
EXEC Sp_Update_CtKT 'E', @PH, @Ct, 'A01010000010064-', 'A01'
*/
IF OBJECT_ID('Sp_Update_CtKT') IS NOT NULL DROP PROCEDURE Sp_Update_CtKT
GO 

CREATE PROCEDURE [dbo].[Sp_Update_CtKT]
(
	@strNew_Edit CHAR(1),
	@PH TVP_PHKT READONLY,
	@Ct TVP_CtKT READONLY,
	@Stt VARCHAR(15),
	@Ma_Ct VARCHAR(5),
	@Ma_DvCs VARCHAR(5)
)
...
BEGIN
	...
	BEGIN
		...
		RETURN
	END
	-- Kiểm tra phiếu KT không thể có mã đối tượng có
	--IF(@Ma_Ct = 'KT' AND EXISTS(SELECT Ma_Dt_Co FROM @Ct WHERE Ma_Dt_Co <> ''))
	--BEGIN
	--	SET @_Stt0_Error = (SELECT MAX(Stt0) FROM @Ct WHERE Ma_Dt_Co <> '' )
	--	RAISERROR (N'Tồn tại dòng [%d] phiếu kế toán không thể có mã đối tượng có', 16, 1, @_Stt0_Error)
	--	RETURN
	--END
	---- Kiểm tra phiếu BT phải có mã đối tượng có
	--IF(@Ma_Ct = 'BT' AND EXISTS(SELECT Ma_Dt_Co FROM @Ct WHERE Ma_Dt_Co = ''))
	--BEGIN
	--	SET @_Stt0_Error = (SELECT MAX(Stt0) FROM @Ct WHERE Ma_Dt_Co = '' )
	--	RAISERROR (N'Tồn tại dòng [%d] phiếu bù trừ phải có mã đối tượng có', 16, 1, @_Stt0_Error)
	--	RETURN
	--END
