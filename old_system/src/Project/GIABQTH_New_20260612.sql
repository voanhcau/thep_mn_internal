...
BEGIN
	...
	UPDATE T1 SET Gia = T2.Gia_Tb, Gia_Nt = T2.Gia_Tb, Gia_Nt9 = T2.Gia_Tb,
		TIEN = So_Luong * T2.Gia_Tb, Tien_Nt = So_Luong * T2.Gia_Tb, Tien_Nt9 = So_Luong * T2.Gia_Tb
		FROM R04CTHD T1 JOIN #T_GiaTb T2 ON T1.Ma_Kho = T2.Ma_Kho AND T1.Ma_Vt = T2.Ma_Vt
		WHERE Auto_Cost = 1 AND Ngay_Ct >= @Ngay_Ct1 AND Ngay_Ct <= @Ngay_Ct2 --AND Tk_No LIKE '632%'
...
--exec [dbo].[sp_GiaBQTH_New] '20260201','20260228', '' --9s
--'04TP' --3s
--'055POM1' --2s
--'052TMN' -- 4s
