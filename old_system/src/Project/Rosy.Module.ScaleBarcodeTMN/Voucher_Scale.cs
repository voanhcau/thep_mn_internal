...
namespace RosyModule.ScaleBarcodeTMN
{
	public static class Voucher_Scale
	{
        ...
        public static bool SQLUpdateCt(frmVoucher_Scale_Edit frmEditCt)
        {
            ...
            if (frmEditCt.drDmCt["Table_Ct"].ToString().ToUpper() == "R05CTN_BARCODE_KKV")
            {
                sqlCom.CommandText = "sp_Update_CtN_Barcode_KKV";

                //TVP_PH
                paraPH.TypeName = "TVP_PH_NBARCODE_KKV";
                paraPH.Value = Voucher.GetTVPValue("R80PH_BARCODE_KKV", "TVP_PH_NBARCODE_KKV", frmEditCt.dtEditPh);
                sqlCom.Parameters.Add(paraPH);

                //TVP_CT
                paraCt.TypeName = "TVP_CT_NBARCODE_KKV";
                paraCt.Value = Voucher.GetTVPValue("R05CTN_BARCODE_KKV", "TVP_CT_NBARCODE_KKV", frmEditCt.dtEditCt);
        ...
        public static void InheritVoucher_SetData_Barcode(frmInherit_Xuat_Barcode frmInherit, frmVoucher_Scale_Edit frmEdit)
        {
            ...
            foreach (DataRow drSelect in frmInherit.dtInherit.Select("Chon = true"))
            {
                ...
                iStt0 += 1;
                //hàng trả lại KKV
                if(frmEdit.strMa_Ct == "PTLKV")
                {
                    // XỬ LÝ ĐÁNH DẤU HÀNG TRẢ LẠI
                    drEditCtNew["Is_TL"] = true;
                    drEditCtNew["Is_PP"] = false;
                    drEditCtNew["Ngay_XLNTL"] = Library.StrToDate("19000101");
                }
		...
		{
            string strSQL = @"SELECT ISNULL(MAX(CAST(RTRIM(LTRIM(So_Ct)) AS BIGINT)), 0) FROM R80PH_BARCODE_KKV WHERE YEAR(Ngay_Ct) = YEAR(GETDATE()) AND Ma_Ct = 'PTLKV'";
			long iStt = Convert.ToInt64(SQLExec.ExecuteReturnValue(strSQL)) + 1;
