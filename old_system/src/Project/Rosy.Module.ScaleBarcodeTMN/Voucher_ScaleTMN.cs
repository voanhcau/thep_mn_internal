...
{
	public static class Voucher_ScaleTMN
	{
        //SQLUpdateCt
        public static bool SQLUpdateCt(frmVoucher_BarCode_Edit frmEditCt)
        {
            ...
            if (frmEditCt.drDmCt["Table_Ct"].ToString().ToUpper() == "R05CTN_BARCODE")
            {
                ...
                paraPH.Value = GetTVPValue("R80PH_BARCODE_KKV", "TVP_PH_SCALE_N", frmEditCt.dtEditPh);
                sqlCom.Parameters.Add(paraPH);

                //Tạo Table cho TVP_CtHD
                paraCt.TypeName = "TVP_CTN_BARCODE";
                paraCt.Value = GetTVPValue("R05CTN_BARCODE_KKV", "TVP_CTN_BARCODE", frmEditCt.dtEditCt);
            ...
            try
            {
                ...
                Voucher_ScaleTMN.Update_dsVoucher(frmEditCt);
            }
            catch (Exception ex)
            {
        ...
        public static bool SQLDeleteCt(string strStt, string strMa_Ct)
        {
            ...
            {
                string strCreate_User = (string)SQLExec.ExecuteReturnValue("SELECT ISNULL(MAX(Create_Log), '') FROM R80PH_BARCODE_KKV WHERE Stt = '" + strStt + "'");
        ...
        public static void Update_Header(frmVoucher_BarCode_Edit frmEditCt)
        ...
        public static void Update_Detail(frmVoucher_BarCode_Edit frmEditCt)
        ...
        public static void Update_Detail(frmVoucher_BarCode_Edit frmEditCt, string strColumnList)
        ...
        public static bool AddRow(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        public static void CopyNewRow(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        public static void DeleteRow(frmVoucher_BarCode_Edit frmEditCt, dgvVoucher dgvEditCt)
        {
        ...
        public static void Update_Stt(frmVoucher_BarCode_Edit frmEditCt)
        {//Kiem tra frmEditCt.strStt co bi trung khong, roi update cho cac table lien quan
            ...
            if (frmEditCt.enuNew_Edit == enuEdit.New || frmEditCt.enuNew_Edit == enuEdit.Copy)
            {
                ...
                {
                    frmEditCt.strStt = Voucher_ScaleTMN.GetNewStt("80");
                }
        ...
        public static void Update_dsVoucher(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        public static void UpdateSo_Ct(frmVoucher_BarCode_Edit frmEditCt)
        {
            if (frmEditCt.enuNew_Edit == enuEdit.New || frmEditCt.enuNew_Edit == enuEdit.Copy)
            {
                frmEditCt.drEditPh["So_Ct"] = Voucher_ScaleTMN.GetNewSo_Ct();
        ...
        public static string GetInheritScaleBarcode(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        public static void Update_DmNvu(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        public static void Update_Log(frmVoucher_BarCode_Edit frmEditCt)
        {
        ...
        //public static void InheritVoucher_SetData_Barcode(frmInherit_Xuat_Barcode frmInherit, frmVoucher_BarCode_Edit frmEdit)
        //{
        ...
        public static string GetNewStt(string strModule)
        {
            ...
            string strSQL = @"SELECT ISNULL(MAX(SUBSTRING(Stt, 6, 10)), 0)
								FROM R80PH_BARCODE_KKV WITH(NOLOCK)
		...
		{
            string strSQL = @"SELECT ISNULL(MAX(CAST(RTRIM(LTRIM(So_Ct)) AS BIGINT)), 0) FROM R80PH_BARCODE_KKV WHERE YEAR(Ngay_Ct) = YEAR(GETDATE())";
			long iStt = Convert.ToInt64(SQLExec.ExecuteReturnValue(strSQL)) + 1;
			string So_Ct = iStt.ToString().Trim().PadLeft(6, '0');
			return So_Ct;
		}

        // kiểm tra in CNXX điện tử
        public static string NormalizeSoChungTu(string input)
        {
            return input.Replace("/", "").Replace("\\", "").Replace(" ", "").Trim();
            //return input.Replace("/", "").Replace("\\", "").Replace("-", "").Replace(" ", "").Trim();
        }

        public static bool TryExtractDocDate(string soChungTu, out DateTime docDate)
        {
            docDate = default(DateTime);
            if (string.IsNullOrEmpty(soChungTu)) return false;

            // Ưu tiên: 8 số ngay sau dấu / hoặc \
            Match m = Regex.Match(soChungTu, @"[/\\](?<d>\d{8})");
            if (!m.Success)
            {
                // Fallback: lấy cụm 8 số đầu tiên xuất hiện (yyyyMMdd)
                m = Regex.Match(soChungTu, @"(?<!\d)(?<d>\d{8})(?!\d)");
            }
        ...
        public static IEnumerable<string> GetPreferredFolders(string baseRoot, DateTime docDate)
        {
            string yearRoot = Path.Combine(baseRoot, docDate.Year.ToString());
            string monthRoot = Path.Combine(yearRoot, "T" + docDate.Month.ToString("00"));

            // ưu tiên ngày và ngày+1
            DateTime[] days = new[] { docDate.Date, docDate.Date.AddDays(1) };
            foreach (var d in days)
            {
                // theo ví dụ của bạn: MM-dd-yyyy (12-18-2025)
                string dayFolder = d.ToString("MM-dd-yyyy");
                yield return Path.Combine(monthRoot, dayFolder);
            }

            // fallback
            yield return monthRoot;
            yield return yearRoot;
        }

        public static List<string> SearchPdfOptimized(string baseRoot, string token, DateTime docDate)
        {
            foreach (var dir in GetPreferredFolders(baseRoot, docDate))
            {
                if (!Directory.Exists(dir)) continue;

                var found = FindPdfFilesByTokenSafe(dir, token);

                // thấy là dừng để nhanh nhất
                if (found.Count > 0) return found;
            }
            return new List<string>();
        }

        public static List<string> FindPdfFilesByTokenSafe(string rootDir, string token)
        {
            string pattern = "*" + token + "*.pdf";
            ...
            while (stack.Count > 0)
            {
                ...
                try
                {
                    // .NET 4.0 có EnumerateFiles, nếu bạn muốn an toàn hơn có thể dùng GetFiles
        ...
        public static string GetNewStt_KKV(string strModule, bool bSaveNew, string strKV)
        {
            DateTime DateServer = Voucher.GetDateServer();
            string strStt = "A01" + DateServer.ToString("yyMM") ;
            if(SQLExec.ExecuteReturnValue("SELECT MAX(RIGHT(Stt,4)) FROM R80PH_BARCODE_KKV WHERE Stt LIKE '" + strStt  + "%'") == DBNull.Value)
                Parameters.SetParaValue("Stt_KKV", 0);

            string str = RosySystem.Element.Element.sysMa_DvCs.Trim().PadLeft(3, '0');
            
            strModule = strModule.Trim().PadLeft(2, '0');

            long num = Convert.ToInt64(Parameters.GetParaValue("Stt_KKV")) + 1;
            //kiểm tra stt này tồn tại chưa
            string str2 = num.ToString().Trim().PadLeft(6, '0');
            string strStt_ = strStt + strKV + str2;
            
            //kiểm tra xem có stt này chưa
            if (DataTool.SQLCheckExist("R80PH_BARCODE_KKV", "Stt", strStt_))
            {
                num = Convert.ToInt64(SQLExec.ExecuteReturnValue("SELECT cast(MAX(RIGHT(Stt,4)) as INT) FROM R80PH_BARCODE_KKV WHERE Stt LIKE '" + strStt + "%'")) + 1;
                if (bSaveNew)
                {
                    Parameters.SetParaValue("Stt_KKV", num);
                }
                str2 = num.ToString().Trim().PadLeft(6, '0');
            }
            else
            {
                if (bSaveNew)
