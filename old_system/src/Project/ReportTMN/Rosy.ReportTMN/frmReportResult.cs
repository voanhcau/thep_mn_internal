...
namespace RosyReportTMN
{
	public partial class frmReportResult : RosySystem.Control.frmBase
	{
		...
		private void FillData()
		{
            ...
            if (drFilter.Table.Columns.Contains("Login_User"))
			...
			if (DataTool.SQLCheckExist("R00Object", "Object_ID", "ACCESS_PRICE") && !Common.CheckPermission("ACCESS_PRICE", enuPermission_Type.Allow_Access) &&
                   !DataTool.SQLCheckExist("R00PERMISSIONBC", new string[] { "Report_ID", "Member_ID", "Allow_Access" }, new object[] { drReport["Report_ID"].ToString(), Element.sysUser_Id, 1 }))  ////Bằng ktra phân quyền cho thấy giá tiền có trong R00PERMISSIONBC
			{
				foreach (DataColumn dc in dtDetail.Columns)
				{
					if (dc.ColumnName.StartsWith("GIA") || dc.ColumnName.StartsWith("TIEN") || dc.ColumnName.StartsWith("PS_NO") || dc.ColumnName.StartsWith("PS_CO") || dc.ColumnName.StartsWith("PS_TANG") || 
                        dc.ColumnName.StartsWith("PS_GIAM") || dc.ColumnName.StartsWith("DU_DAU") || dc.ColumnName.StartsWith("DU_CUOI") || dc.ColumnName.StartsWith("DU_NO") || dc.ColumnName.StartsWith("DU_CO") ||
                        dc.ColumnName.StartsWith("NAM_NAY") || dc.ColumnName.StartsWith("NAM_TRUOC"))
					{
		...
		public void Export() //Phục vụ cho việc gọi Assemply từ ToolStrip
		{
			...
			//KIỂM TRA USER CÓ DC THÊM KHÔNG
			//string strObject = SQLExec.ExecuteReturnValue("SELECT Object_ID FROM R00MENU_REPORT WHERE Report_ID = '" + drReport["Report_ID"].ToString() + "'").ToString();
			//if(!Common.CheckPermission(strObject, enuPermission_Type.Allow_View))
			//{
			if ((string)drReport["SQLProc"] != "sp_rptPXHDExcel")
				RosyCommonTMN.CommonTMN.ExportXLSX(ExportControl, strTitle, strSubtitle);//Common.Export(ExportControl, strTitle, strSubtitle); 
			else
				RosyCommonTMN.CommonTMN.Export(ExportControl, strTitle, strSubtitle, "REPORT");
            //}
		}
