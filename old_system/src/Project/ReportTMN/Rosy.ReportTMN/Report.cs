...
namespace RosyReportTMN
{
	public class Report
	{
        ...
        private static DataRow GetdrFilterAll(frmReportAll frmReport, DataRow drReport)
        {//Tra ve DataRow chua cac tham so cua Report
        ...
        public static void RunReportAll(frmReportAll frmReport)
        {
            if (frmReport.bdsReport.Current == null || frmReport.bdsReport.Position < 0)
                return;
            ...
            //Tao tham so bao cao
            drFilter = GetdrFilterAll(frmReport, drReport);
		...
		public static string GetSubTitle1(DataRow drReport, DataRow drFilter)
		{
			...
			{
                DateTime dteNgay_Ct1 = Library.StrToDate(drFilter["NGAY_CT1"].ToString());// (DateTime)drFilter["NGAY_CT1"];
                DateTime dteNgay_Ct2 = Library.StrToDate(drFilter["NGAY_CT1"].ToString()); // (DateTime)drFilter["NGAY_CT2"];
				strDay1 = dteNgay_Ct1.Day.ToString();
				strMonth1 = dteNgay_Ct1.Month.ToString();
				strYear1 = dteNgay_Ct1.Year.ToString();
				strDay2 = dteNgay_Ct2.Day.ToString();
				...
				if (strDay1 == "1" && strYear1 == strYear2)
				{
					if (strMonth1 == "1" && strMonth2 == "3" && strDay2 == "31")
                        ...
                        return Common.ReadDate(Library.StrToDate(drFilter["NGAY_CT1"].ToString()), Library.StrToDate(drFilter["NGAY_CT2"].ToString()), Element.sysLanguage);//return Common.ReadDate((DateTime)drFilter["NGAY_CT1"], (DateTime)drFilter["NGAY_CT2"], Element.sysLanguage);
				}
                //return Common.ReadDate((DateTime)drFilter["NGAY_CT1"], (DateTime)drFilter["NGAY_CT2"], Element.sysLanguage);
                return Common.ReadDate(Library.StrToDate(drFilter["NGAY_CT1"].ToString()), Library.StrToDate(drFilter["NGAY_CT2"].ToString()), Element.sysLanguage);
			}
			else if (drFilter.Table.Columns.Contains("NGAY_CT"))
			{
                return Common.ReadDate(Library.StrToDate(drFilter["NGAY_CT"].ToString()), Library.StrToDate(drFilter["NGAY_CT"].ToString()), Element.sysLanguage);
			}
		...
		public static string GetSubTitle2(DataRow drReport, DataRow drFilter)
		{
			...
			foreach (string strFilter_ID in strArrSubTile)
			{
				...
				//Bằng lấy giá trị của điều kiện subtitle
				string strSubTitle_ = string.Empty;
				strSubTitle_ = strLabel + ": " + strFilter_ID_Value + " - " + strFilter_Name_Value;

				if (strSubTitle == string.Empty)
					strSubTitle = strLabel + ": " + strFilter_ID_Value + " - " + strFilter_Name_Value;
				else
					strSubTitle += "|" + strSubTitle_;
				//Bằng bỏ phần này
