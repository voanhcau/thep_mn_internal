...
{
    public partial class frmThucDon : RosySystem.Customize.frmView
	{
		#region Variable
        ...
        public frmThucDon()
		{
            ...
            dgvThucDon.Enter += new EventHandler(dgvNhanVienCa_Enter);
            dgvThucDonNV.Enter += new EventHandler(dgvNhanVienCty_Enter);
            dgvThucPham.Enter += new EventHandler(dgvThucPham_Enter);
		...
		private void Build()
		{
            ...
            dgvThucDon.bSortMode = false;
            dgvThucDon.strZone = "THUCDON";
            dgvThucDon.BuildGridView();


            dgvThucDonNV.bSortMode = false;
            dgvThucDonNV.strZone = "THUCDONNV";
            dgvThucDonNV.BuildGridView();

            dgvThucPham.bSortMode = false;
            dgvThucPham.strZone = "THUCDONDT";
            dgvThucPham.BuildGridView();

            this.DataGridView_Language();

            foreach (DataGridViewColumn dgvc in dgvThucDon.Columns)
                dgvc.ReadOnly = true;


            foreach (DataGridViewColumn dgvc in dgvThucDonNV.Columns)
                dgvc.ReadOnly = true;


            foreach (DataGridViewColumn dgvc in dgvThucPham.Columns)
                dgvc.ReadOnly = true;
	    ...
	    private void FillData()
		{
            ...
            dsVoucher = SQLExec.ExecuteReturnDs("sp_GetThucDon", ht, CommandType.StoredProcedure);

            dtEditCt_ThucDon = dsVoucher.Tables[0];
            bdsEditCt_ThucDon.DataSource = dtEditCt_ThucDon;
            dgvThucDon.DataSource = bdsEditCt_ThucDon;

            dtEditCt_ThucDonNV = dsVoucher.Tables[1];
            bdsEditCt_ThucDonNV.DataSource = dtEditCt_ThucDonNV;
            dgvThucDonNV.DataSource = bdsEditCt_ThucDonNV;

            dtEditCt_ThucPham = dsVoucher.Tables[2];
            bdsEditCt_ThucPham.DataSource = dtEditCt_ThucPham;
            dgvThucPham.DataSource = bdsEditCt_ThucPham;
        ...
        void dgvNhanVienCa_Enter(object sender, EventArgs e)
        {
            ...
            objActive = dgvThucDon;
        }
        void dgvNhanVienCty_Enter(object sender, EventArgs e)
        {
            ExportControl = sender;
            objActive = dgvThucDonNV;
        }
        void dgvThucPham_Enter(object sender, EventArgs e)
        {
            ExportControl = sender;
            objActive = dgvThucPham;
        }
        private void Save()
        {
        ...
        void btTinh_Click(object sender, EventArgs e)
        {
            Hashtable ht = new Hashtable();
            ...
            SQLExec.Execute("sp_TinhSuatAn", ht, CommandType.StoredProcedure);
            FillData();
        ...
        private void Design()
        {
            string strReportFile = "rptThucDon";
