	...
	public partial class frmCtNXTH : RosySystem.Customize.frmView
	{
		...
		private void FillData()
		{
			...
			if (this.strMa_Ca != string.Empty)
				dtDmCa = DataTool.SQLGetDataTable("R81DMCA", "", "Ma_Ca = '" + strMa_Ca + "' AND Loai = 'CAN'", "");
			else
                dtDmCa = DataTool.SQLGetDataTable("R81DMCA", "", "Ended = 0 AND Loai = 'CAN'", "");

			//Lấy ca cuối cùng
			if (dtDmCa.Rows.Count == 0)
			{
                string strSQLExec = "SELECT ISNULL(MAX(CAST(RTRIM(LTRIM(Ma_Ca)) AS BIGINT)), 0) FROM R81DMCA WHERE YEAR(Ngay_Sx) = " + Voucher.GetDate_Server().Year + " AND Ended = 1 AND Loai = 'CAN'";
				string strMa_Ca_KT = Convert.ToString(SQLExec.ExecuteReturnValue(strSQLExec, CommandType.Text));
                dtDmCa = DataTool.SQLGetDataTable("R81DMCA", "", "Ma_Ca = '" + strMa_Ca_KT + "' AND Loai = 'CAN'", "");
			...
			if (this.strMa_Ca != string.Empty)
			{
                string strSQLExec = "SELECT ISNULL(MAX(CAST(RTRIM(LTRIM(Ma_Ca)) AS BIGINT)), 0) FROM R81DMCA WHERE YEAR(Ngay_Sx) = " + Library.StrToDate(dteNgay_Sx.Text).Year + " AND Ended = 1 AND Loai = 'CAN'";
				string strMa_Ca_KT = Convert.ToString(SQLExec.ExecuteReturnValue(strSQLExec, CommandType.Text));
        ...
        private void Insert_Barcode()
		{
            if (!bTestCan)
            {
                ...
                else
                {
					...
					htPara.Add("XUONG", "0"); //tmn đang chỉ sx 1 da
            ...
            {
                double dbSo_Luong = 0.0; string strLoaiCan = "";
                dbSo_Luong = dbWeight;
                DataTable dtEquipment = DataTool.SQLGetDataTable("R81EquipmentInfo", "*", "Host_IP = '" + MachineInfo.GetHostIP() + "'", "Host_IP");

                if (dtEquipment.Rows.Count > 0)
                    strLoaiCan = dtEquipment.Rows[0]["Loai_Sp"].ToString();

                dbSo_Luong = dbSo_Luong - Variables.dbZero_Coefficient;

                Hashtable htTestcan = new Hashtable();
                htTestcan.Add("STRNEW_EDIT", "N");
                htTestcan.Add("NGAY_CAN", Voucher.GetDate_Server());
                htTestcan.Add("GIO_CAN", Voucher.GetDateServer().ToString("HH:mm:ss"));
                htTestcan.Add("LOAI_SP", strLoaiCan);
                htTestcan.Add("MA_CA", txtMa_Ca.Text);
                htTestcan.Add("SO_LUONG_CAN", dbSo_Luong);
                htTestcan.Add("GHI_CHU", txtGhi_Chu.Text);
                htTestcan.Add("CREATE_LOG", Common.GetCurrent_Log());

                DataTable dtTestCan0 = SQLExec.ExecuteReturnDt("sp_Update_TestCan", htTestcan, CommandType.StoredProcedure);
                if (dtTestCan0.Rows.Count > 0)
                {
                    DataRow drAddCurrent = dtTestCan0.Rows[0];

                    DataRow drEditCtNew = dtTestCan.NewRow();
                    Common.CopyDataRow(drAddCurrent, drEditCtNew);
                    Common.SetDefaultDataRow(ref drEditCtNew);

                    dtTestCan.Rows.Add(drEditCtNew);
                    drEditCtNew.AcceptChanges();
                }

                bdsTestCan.Sort = "GIO_CAN DESC";
                bdsTestCan.Position = 0;
            }
		}
		...
		private bool CheckExistsCode(int iCode)
		{
			string strSQLExec = @"SELECT Code FROM R81DMBARCODE T1 WITH(NOLOCK) JOIN (SELECT Ma_Ca FROM R81DMCA WITH(NOLOCK) WHERE YEAR(Ngay_Sx) = " + DateTime.Parse(dteNgay_Sx.Text).Year + ") T2 ON T1.Ma_Ca = T2.Ma_Ca" + @"
										--JOIN R81DMSIZE T3 WITH(NOLOCK) ON T1.Ma_Size = T3.Ma_Size
									WHERE T1.Code = " + iCode + " AND T1.Barcode NOT LIKE 'L%' AND So_Ct_LXH = ''";
		...
		private object GetNewCode()
		{
			string strSQLExec = @"SELECT ISNULL(MAX(CAST(RTRIM(LTRIM(Code)) AS BIGINT)), 0)
										FROM R81DMBARCODE T1 WITH(NOLOCK) JOIN (SELECT Ma_Ca FROM R81DMCA WITH(NOLOCK) WHERE  YEAR(Ngay_Sx) = " + DateTime.Parse(dteNgay_Sx.Text).Year + ") T2 ON T1.Ma_Ca = T2.Ma_Ca WHERE Barcode NOT LIKE 'L%' AND So_Ct_LXH = ''";
		...
		public override void Edit(enuEdit enuNew_Edit)
		{
			...
			{
				if (frm.bPrint)
					Voucher.PrintBarcode(drCurrent["Barcode"].ToString(), Convert.ToBoolean(drCurrent["Is_Barem"]), false, Variables.strPrint_Barcode, false);
		...
		void numCode_Validated(object sender, EventArgs e)
		{
            ...
            int iCodeMax = Convert.ToInt32(SQLExec.ExecuteReturnValue("SELECT MAX(Code) FROM R81DMBARCODE T1 WITH(NOLOCK) JOIN (SELECT Ma_Ca FROM R81DMCA WITH(NOLOCK) " + @"
                                                           WHERE YEAR(Ngay_Sx) = " + DateTime.Parse(dteNgay_Sx.Text).Year + ") T2 ON T1.Ma_Ca = T2.Ma_Ca WHERE Barcode NOT LIKE 'L%' "));
		...
		{
			//kiểm tra IP của máy
			string strLocalIP = MachineInfo.GetHostIP();
			string strLoai = "THANH";
			string strIP = "192.168.10.31";
			
			string strLoai_Check = string.Empty;
			// hỏi xem cân của IP thanh hay cuộn
			DataTable dtEquipment = DataTool.SQLGetDataTable("R81EquipmentInfo", "*", "Host_IP = '" + MachineInfo.GetHostIP() + "'", "Host_IP");
			if (dtEquipment.Rows.Count > 0)
				strLoai = dtEquipment.Rows[0]["Loai_Sp"].ToString();

			if (strLoai == "THANH")
			{ strLoai_Check = "CUON"; strIP = "192.168.11.155"; }
			else if (strLoai == "CUON")
				strLoai_Check = "THANH"; strIP = "192.168.11.154";

			//if (Common.MsgYes_No("Bạn đang cán  " + strLoai + ", bạn có muốn đổi IP sang cán " + strLoai_Check + " không ?", "Y"))
			//	Voucher.SetIPAddress(strIP, "192.168.11.1", "192.168.1.4", "192.168.1.35");
		}
        ...
        void btCbTest_Click(object sender, EventArgs e)
        {
            ...
            {
                bTestCan = true;
                btCbTest.Text = "Test cân xong";
				btCbTest.ForeColor = Color.Red;
				btCbTest.Font = new Font("Microsoft Sans Serif", 10, FontStyle.Bold);
				//lấy dữ liệu đầu cân import phần mềm



			}
            else if (btCbTest.Text == "Test cân xong")
            {
                bTestCan = false;
                btCbTest.Text = "Chuẩn bị Test cân";
				btCbTest.ForeColor = Color.Blue;
				btCbTest.Font = new Font("Microsoft Sans Serif", 10, FontStyle.Bold);
			}
        }
        void Edit_TestCan(enuEdit enuNew_Edit)
        {
            ...
            if (dtEquipment.Rows.Count > 0)
                drCurrent["Loai_Sp"] = dtEquipment.Rows[0]["Loai_Sp"].ToString();
            else
                drCurrent["Loai_Sp"] = "THANH";
