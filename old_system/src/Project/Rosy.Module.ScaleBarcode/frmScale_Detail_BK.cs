...
{
	public partial class frmScale_Detail_BK : RosySystem.Customize.frmView
	{
		#region Variable
		...
		private void Calc_So_Luong(bool bThep_Cay)
		{
			...
			DataView dvEditCt = new DataView(dtEditCt);
			dvEditCt.RowFilter = "Deleted = 0";
			numTSo_Barcode_Thanh.Value = dvEditCt.ToTable().Rows.Count;
...
//														FROM R05CTX_BARCODE WITH(NOLOCK) WHERE Stt <> '" + this.strStt + "' AND Barcode = '" + strBarcode + "' GROUP BY Barcode) T2 ON T1.Barcode = T2.Barcode" + @"
//										LEFT JOIN(SELECT T2a.Barcode, ISNULL(SUM(T2a.So_Luong), 0) AS So_Luong_TL, ISNULL(SUM(T2a.Num_Bars), 0) AS Num_Bars_TL
//														FROM R81DMBARCODE T1a WITH(NOLOCK) JOIN R05CTN_BARCODE T2a WITH(NOLOCK) ON T1a.Barcode = T2a.Barcode
//														WHERE T2a.Barcode = '" + strBarcode + "' AND T1a.Is_OutPut = 1" + @"
//														GROUP BY T2a.Barcode) T3 ON T1.Barcode = T3.Barcode" + @"
...
//								WHERE T1.Barcode = '" + strBarcode + "' AND T1.So_Luong + ISNULL(T3.So_Luong_TL, 0) - ISNULL(T2.So_Luong, 0) <> 0";
					...
					else
					{
						...
						string strMa_Vt_Org = SQLExec.ExecuteReturnValue("SELECT Ma_Vt_Org FROM R80PH_SCALE WITH(NOLOCK) WHERE Stt = '" + this.strStt + "'").ToString();
