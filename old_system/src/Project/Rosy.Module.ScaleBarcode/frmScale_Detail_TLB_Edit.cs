	...
	public partial class frmScale_Detail_TLB_Edit : frmVoucher_Scale_Edit
	{
        ...
        public override void Load(enuEdit enuNew_Edit, DataRow drEdit, DataSet dsVoucher)
		{
			...
			if (enuNew_Edit == enuEdit.New || enuNew_Edit == enuEdit.Copy)
				this.strStt = Voucher_Scale.GetNewStt("80");
			else
		...
		private void FillData()
		{
			...
			DataSet dsVoucher = SQLExec.ExecuteReturnDs("sp_GetVoucher_Scale_CtTLB", htPara, CommandType.StoredProcedure);
		...
		private void ReUpdate_Status_KCS()
		{
			...
			foreach (DataRow dr in dtEditCt.Rows)
			{
				...
				if (!SQLExec.Execute("sp_ReUpdate_TLB_Status_KCS", htPara, CommandType.StoredProcedure))
				{
