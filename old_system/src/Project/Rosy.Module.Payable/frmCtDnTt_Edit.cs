...
{
	public partial class frmCtDnTt_Edit : frmVoucher_Edit
	{
        ...
        public frmCtDnTt_Edit()
		{
			...
			txtMa_Bp_Sd.Validating += new CancelEventHandler(txtMa_Bp_Sd_Validating);
			txtMa_Tte.Leave += new EventHandler(txtMa_Tte_Leave);
			numTy_Gia.Leave += new EventHandler(numTy_Gia_Leave);
            txtDien_Giai.Leave += TxtDien_Giai_Leave;
        ...
        private void DataGridView_Language()
        {
            ...
            if (enuNew_Edit == enuEdit.New || enuNew_Edit == enuEdit.Copy)
            {
                ...
                drEdit["Ma_Bp_Sd"] = (string)SQLExec.ExecuteReturnValue("SELECT MIN(Member_Group_ID) FROM R00MEMBERGROUP WHERE Member_Id = '" + Element.sysUser_Id + "' AND Member_Group_ID NOT IN ('TDVSX','BARCODEPH')");
                if(Common.Inlist(drEdit["Ma_Bp_Sd"].ToString(), "PXCAN,PXLUYEN"))
					txtMa_Bp_Sd.Text = "PXSX";
				else
					txtMa_Bp_Sd.Text = drEdit["Ma_Bp_Sd"].ToString();
            }

            if (Common.Inlist(strMa_Ct, "DNTT"))
            {
                if (dgvEditCt1.Columns.Contains("Ngay_Ky"))
                    dgvEditCt1.Columns["Ngay_Ky"].HeaderText = "Ngày nhận HS của NCC";

				dgvEditCt1.Columns["Tien_Nt9"].HeaderText = "Tiền hàng";
				dgvEditCt1.Columns["Tien_Nt3"].HeaderText = "Tiền thuế";
				dgvEditCt1.Columns["Tien_Tu_Nt"].HeaderText = "Tiền tạm ứng";
				dgvEditCt1.Columns["TIEN_TT_NT"].HeaderText = "Tiền thanh toán";
			}
        }
		...
		private void Init_Ct()
		{
			...
			if (enuNew_Edit == enuEdit.New || enuNew_Edit == enuEdit.Copy)
			{
				if (this.enuNew_Edit == enuEdit.New)
				{
                    ...
                    drCurrent["Ngay_Ct"] = DateTime.Now; //drEdit["Ngay_Ct"] != DBNull.Value ? drEdit["Ngay_Ct"] :
                    ...
                    drEditPh["Ngay_Ct"] = DateTime.Now;// drCurrent["Ngay_Ct"];
            ...
            numLoai_Ct.Text = drCurrent["Loai_Ct"].ToString();
            drEditPh["Loai_Ct"] = drCurrent["Loai_Ct"];
		...
		private void LoadDicName()
		{
            ...
            //txtMa_Bp
            if (txtMa_Bp_Sd.Text.Trim() != string.Empty)
            {
                lbtTen_Bp.Text = DataTool.SQLGetNameByCode("R81DmBp", "Ma_Bp", "Ten_Bp", txtMa_Bp_Sd.Text.Trim());
            }
            else
		...
		private bool FormCheckValid()
		{
           ...
           if(numLoai_Ct.Text == "")
           {
               string strMsg = Element.sysLanguage == enuLanguageType.Vietnamese ? "Chưa khai báo loại chứng từ là thanh toán hay tạm ứng" : "Do not register transaction type";
               Common.MsgCancel(strMsg);
               return false;
           }
			//kiểm tra ngày thanh toán có rơi vào t7 và cn hay không
			double iHan = Convert.ToDouble(numHan_Tt.Text);
			DateTime dtNgay_Ct0 = Convert.ToDateTime(dtEditCt.Compute("MAX(Ngay_Ct0)",""));
			if(dtNgay_Ct0 == Convert.ToDateTime("01/01/1900"))
				dtNgay_Ct0 = Convert.ToDateTime(dtEditCt.Compute("MAX(Ngay_Ct)", ""));
			Hashtable h1 = new Hashtable();
			h1.Add("NGAY_CT0", dtNgay_Ct0);
			h1.Add("HAN", iHan);
			h1.Add("NGAY_CT01", dtNgay_Ct0);
			h1.Add("HAN1", iHan);
			DataTable dt1 = SQLExec.ExecuteReturnDt("SELECT DATEADD(DAY,@Han,@Ngay_Ct0) as Ngay, UPPER(DATENAME(DW,DATEADD(DAY,@Han1,@Ngay_Ct01))) as Ten", h1,CommandType.Text);
			DateTime dtNgay_Ct = Convert.ToDateTime(dt1.Rows[0]["Ngay"]);
			string strNameDay = Convert.ToString(dt1.Rows[0]["Ten"]);
			if (Common.Inlist(strNameDay,"SATURDAY,SUNDAY") && Library.DateToStr(dtNgay_Ct0) != "")
            {
				Common.MsgOk("Ngày đến hạn thanh toán " + dtNgay_Ct.ToShortDateString() + " rơi vào " + strNameDay + " không thanh toán được!!! anh chị kiểm tra lại.");
				return false;
            }
			if(dtNgay_Ct <= Convert.ToDateTime(drEditPh["Ngay_Ct"]) && strMa_Ct == "DNTT")
			{
				Common.MsgOk("Ngày đến hạn thanh toán " + dtNgay_Ct.ToShortDateString() + " trước hoặc bằng ngày chứng từ!!! anh chị kiểm tra lại cần nhập thông tin hạn thanh toán.");
				return false;
			}
			...
			foreach (DataRow dr in dtEditCt.Rows)
			{
                ...
                if (dr["So_Ct0"].ToString() == "" && Convert.ToDouble(dr["Tien_Nt9"]) == 0)
                    dr["Deleted"] = true;
				//kiểm tra thông tin thuế
				if (dr["So_Ct0"].ToString() != "" && dr["So_Seri0"].ToString() != "" &&
					(dr["Ma_So_Thue"].ToString() == "" || dr["Ten_DtGtGt"].ToString() == "" || dr["Ma_Thue"].ToString() == ""))
				{
					Common.MsgOk("Thông tin thuế của chứng từ thiếu thông tin, các anh chị kiểm tra lại thông tin trước khi lưu!!!");
					return false;
				}
				if(Convert.ToDateTime(dr["Ngay_Ct0"]).ToShortDateString() != Convert.ToString("01/01/1900") 
						&& dr["So_Ct0"].ToString() == "" && dr["So_Seri0"].ToString() == "")
				{
					Common.MsgOk("Thông tin ngày hóa đơn cần xóa đi, các anh chị kiểm tra lại thông tin trước khi lưu!!!");
					return false;
				}
				//kiểm tra chiều dài của MST
				string strMa_So_Thue = dr["Ma_So_Thue"].ToString();
				if (strMa_So_Thue != "" && (strMa_So_Thue.Length != 10 && strMa_So_Thue.Length != 12 && strMa_So_Thue.Length != 14))
				{
					Common.MsgOk("Thông tin mã số thuế không hợp lệ, các anh chị kiểm tra lại chiều dài của MST phải 10,12,14 ký tự!!!");
					return false;
				}
		...
		private void TxtDien_Giai_Leave(object sender, EventArgs e)
		{
			if (enuNew_Edit == enuEdit.New)
			{
				foreach (DataRow dr in dtEditCt.Rows)
		...
		void txtMa_Dt_Validating(object sender, CancelEventArgs e)
		{
			...
			else
			{
				...
				if (txtMa_Dt.Text != (string)drEditPh["Ma_Dt"])
				{
                    ...
                    {
                        if (drLookup["Ten_Nh"].ToString() != string.Empty)
                            txtTen_Nh_B.Text = drLookup["Ten_Nh"].ToString();

                        if (drLookup["So_Tk_Nh"].ToString() != string.Empty)
                            txtTk_Nh_B.Text = drLookup["So_Tk_Nh"].ToString();
                    }
                    else
                    {
                        if (drLookup["Ten_Nh"].ToString() != string.Empty)
                            txtTen_Nh_B.Text = drLookup["Ngan_Hang"].ToString();

                        if (drLookup["So_Tk_Nh"].ToString() != string.Empty)
                            txtTk_Nh_B.Text = drLookup["So_Tk"].ToString();
                    }
				}
			}
		...
		void txtMa_Bp_Sd_Validating(object sender, CancelEventArgs e)
        {
            string strValue = txtMa_Bp_Sd.Text.Trim();
            bool bRequire = false;
            ...
            if (drLookup == null)
            {
                txtMa_Bp_Sd.Text = string.Empty;
                lbtTen_Bp.Text = string.Empty;
            }
            else
            {
                txtMa_Bp_Sd.Text = drLookup["Ma_Bp"].ToString();
		...
		void dgvEditCt_CellEnter(object sender, DataGridViewCellEventArgs e)
		{
			...
			{
                if (dgvEditCt1.Columns.Contains("AUTO_COST"))
                {
		...
		void dgvEditCt_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
		{
			...
			if (this.ActiveControl == dgvEditCt || this.ActiveControl.GetType().Name == "DataGridViewTextBoxEditingControl")
			{
				...
				if (Common.Inlist(strColumnName, "SO_SERI0,SO_CT0"))
				{
		...
		void dgvEditCt_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (Common.Inlist(strColumnName, "SO_LUONG9,GIA_NT9,TIEN_NT9,TIEN_NT3,TIEN_TU_NT,TIEN,TIEN_NT4"))
			{
                Voucher.Calc_ChietKhau(drCurrent, this);
        ...
        void Update_Tien_Tt()
        {
			...
			if (Element.sysMa_Tte == txtMa_Tte.Text)
                drCurrent["Tien_Tt_Nt"] = Convert.ToDouble(drCurrent["Tien"]) + Convert.ToDouble(drCurrent["Tien3"]) - Convert.ToDouble(drCurrent["Tien_Tu"]) - Convert.ToDouble(drCurrent["Tien4"]);
            else
                drCurrent["Tien_Tt_Nt"] = Convert.ToDouble(drCurrent["Tien_Nt"]) + Convert.ToDouble(drCurrent["Tien_Nt3"]) - Convert.ToDouble(drCurrent["Tien_Tu_Nt"]) - Convert.ToDouble(drCurrent["Tien_Nt4"]);
        ...
        private void InheritVoucher()
        {
            ...
            if (frmInherit.Is_Accept == true)
            {
				...
				if(frmInherit.dtInheritVoucher.Columns.Contains("Ma_Dt1"))
					txtMa_Dt.Text = frmInherit.dtInheritVoucher.Select("Chon = true")[0]["Ma_Dt1"].ToString();
				else
					txtMa_Dt.Text = frmInherit.dtInheritVoucher.Select("Chon = true")[0]["Ma_Dt"].ToString();

                txtMa_Hd.Text = frmInherit.dtInheritVoucher.Select("Chon = true")[0]["Ma_Hd"].ToString();
                ...
                numTy_Gia.Value = Convert.ToDouble(frmInherit.dtInheritVoucher.Select("Chon = true")[0]["Ty_Gia"]);
                numHan_Tt.Value = Convert.ToDouble(frmInherit.dtInheritVoucher.Select("Chon = true")[0]["Han_Tt"]);
				//if(txtMa_Dt.Text!= "")
				//{ 
                ...
                foreach (DataRow drSelect in frmInherit.dtInheritVoucher.Select("Chon = true"))
                {
                    ...
                    drEditCtNew["So_Ct0"] = drSelect["So_Ct0"];
                    drEditCtNew["Ngay_Ct0"] = drSelect["Ngay_Ct0"];

                    drEditCtNew["Tien_Tu"] = drSelect["Tien_Tu"];
                    drEditCtNew["Tien_Tu"] = drSelect["Tien_Tu_Nt"];
                    drEditCtNew["Tien3"] = drSelect["Tien3"];
                    drEditCtNew["Tien_Nt3"] = drSelect["Tien_Nt3"];
                    drEditCtNew["Tien_Nt9"] = drSelect["Tien_Nt"];
                    drEditCtNew["Tien"] = drSelect["Tien"];
                    drEditCtNew["Tien_Nt"] = drSelect["Tien_Nt"];
                    drEditCtNew["Tien_Tt"] = drSelect["Tien_Tt_Nt"];
                    drEditCtNew["Ma_Thue"] = drSelect["Ma_Thue"];
                    drEditCtNew["Thue_GtGt"] = drSelect["Thue_GtGt"];
					drEditCtNew["So_Seri0"] = drSelect["So_Seri0"];
        ...
        private void rsDateTime1_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {
