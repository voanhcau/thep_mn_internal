...
{
	public partial class frmCtNM_Edit_Vat : frmVoucher_Edit
	{
		...
		private bool FormCheckValid()
		{
			...
			foreach (DataRow dr in dtEditCt.Rows)
			{
				...
				//kiểm tra chiều dài của soseri9
		...
		private void Phan_Bo_Cp()
		{
			...
			Voucher.Update_TTien(this);
			numTSo_Luong.Value = Common.SumDCValue(dtEditCt, "Gia_Tri_Pb", "");
		...
		void txtMa_Dt_Validating(object sender, CancelEventArgs e)
		{
			...
			else
			{
				...
				{
					txtOng_Ba.Text = drLookup["Ten_Dt"].ToString() == string.Empty ? drLookup["Ten_Dt"].ToString() : drLookup["Ten_Dt"].ToString();
		...
		void txtMa_Bp_Validating(object sender, CancelEventArgs e)
		{
			...
			else
			{
				...
				//Cap nhat xuong chi tiet
				if (txtMa_Bp.Text != (string)drEditPh["Ma_Bp"])
				{
					foreach (DataRow dr in dtEditCt.Rows)
					{
						if ((string)dr["Ma_Bp"] == (string)drEditPh["Ma_Bp"])
							dr["Ma_Bp"] = txtMa_Bp.Text;
					}
				}
			}
		}
		...
		void txtMa_Km_Validating(object sender, CancelEventArgs e)
		{
			...
			else
			{
				...
				//Cap nhat xuong chi tiet
				if (txtMa_Km.Text != (string)drEditPh["Ma_Km"])
				{
					foreach (DataRow dr in dtEditCt.Rows)
					{
						if ((string)dr["Ma_Km"] == (string)drEditPh["Ma_Km"])
							dr["Ma_Km"] = txtMa_Km.Text;
					}
				}
			}
		}
		...
		void dgvEditCt_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
		{
			...
			if (this.ActiveControl == dgvEditCt || this.ActiveControl.GetType().Name == "DataGridViewTextBoxEditingControl")
			{
				...
				else if (Common.Inlist(strColumnName, "TK_NO5,TK_CO5,TK_NO6,TK_CO6"))
					bLookup = dgvLookupTk(ref dgvCell, strColumnName);
		...
		private bool dgvLookupMa_Hd(ref DataGridViewCell dgvCell)
		{
			...
			DataRow drLookup = Lookup.ShowLookup("Ma_Hd", strValue, bRequire, "Ma_Dt = '"+txtMa_Dt.Text+"'", "");
		...
		protected override void OnShown(EventArgs e)
		{
			...
			if (this.enuNew_Edit == enuEdit.Edit)
			{
				...
				bool bLock = (bool)drEditPh["Locked"];
